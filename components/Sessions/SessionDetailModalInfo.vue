	...
	<div class="bg-white rounded-lg shadow-lg p-4 md:p-8 mt-4">
		<div class="flex flex-row flex-wrap">
			<div class="flex flex-col w-full md:w-1/2 p-0 md:pr-4">
				...
				<div class="text-xs sm:text-sm mb-8">{{job.platform_job.opportunity_for_catch_up_slots ? 'Yes' : 'No'}}
				</div>
			...
			<div class="flex flex-col w-full md:w-1/2 p-0 md:pl-4">
				...
				<template v-if="job.selection_date">
					<div class="font-bold text-sm sm:text-md">Selection will be made and you will receive a notification
						by this date</div>
					<div class="text-xs sm:text-sm mb-8">
						{{$moment(job.selection_date, 'YYYY-MM-DD[T]HH:mm:ss.SSS[Z]').format('DD-MM-YYYY')}} |
						{{$moment(job.selection_date, 'YYYY-MM-DD[T]HH:mm:ss.SSS[Z]').format('HH:mm')}}</div>
				</template>

				<template v-if="job.platform_job.auto_assign_at">
					<div class="font-bold text-sm sm:text-md mb-8">Auto-assigns this job to the first matching applicant
					</div>
				</template>

				<template v-if="job.favorite_first && job.favorite_only">
					<div class="font-bold text-sm sm:text-md">Only Favorite Locum will be notified until this date</div>
					<div v-if="job.favorite_first && job.favorite_only" class="text-xs sm:text-sm mb-8">
						{{$moment(job.favorite_only_until, 'YYYY-MM-DD[T]HH:mm:ss.SSS[Z]').format('DD-MM-YYYY')}} |
						{{$moment(job.favorite_only_until, 'YYYY-MM-DD[T]HH:mm:ss.SSS[Z]').format('HH:mm')}}</div>
				</template>

				<template v-if="!job.favorite_first && job.favorite_only">
					<div class="font-bold text-sm sm:text-md mb-8">This Job is available for Favorite Locum only</div>
				</template>

				<div class="text-xs sm:text-sm mb-8">
					This job is
					<span class="font-bold text-sm sm:text-md">{{job.platform_job.ir35 ? 'INSIDE' : 'OUTSIDE'}}</span>
					of scope of
				...
				<div class="text-xs sm:text-sm mb-8 flex flex-row flex-wrap">
					<div class="rounded-lg bg-yellow-500 p-1 m-1" v-for="item in job.platform_job.qualifications"
				...
				<div class="text-xs sm:text-sm mb-8 flex flex-row flex-wrap">
					...
					<div v-else class="rounded-lg bg-yellow-500 p-1 m-1"
						v-for="item in job.platform_job.compliance_documents" :key="item.id">{{item.name}}</div>
				</div>
				...
				<div class="text-xs sm:text-sm mb-8 flex flex-row flex-wrap">
					...
					<div v-else class="rounded-lg bg-yellow-500 p-1 m-1"
						v-for="item in job.platform_job.mandatory_trainings" :key="item.id">{{item.name}}</div>
				</div>
