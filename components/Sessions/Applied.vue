...
export default {
  ...
  watch: {
    $route(to, from) {
      ...
      this.getAppliedSession()
    }
  },
  computed: {
    jobs() {
      return this.$store.state.session.appliedJobs
    }
  },
  created() {
    const query = {
  ...
  methods: {
    getAppliedSession() {
      ...
      this.$axios.$get(`/api/v1/practice/jobs?status=Applied&limit=${this.perPage}&offset=${offset}`).then(res => {
        if (res.data.jobs.length === 0 && this.$route.query.current_page !== 1) {
          this.pagechanged(this.$route.query.current_page - 1)
        }
        this.$store.commit('session/SET_APPLIED_JOBS', res.data.jobs)
        this.loading = false
      })
    },
    pagechanged(e) {
