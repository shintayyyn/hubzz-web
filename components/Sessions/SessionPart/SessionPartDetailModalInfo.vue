	...
	<div class="bg-white rounded-lg shadow-lg p-4 md:p-8 mt-4">
        ...
        <div
          class="text-xs sm:text-sm mb-8"
				...
				<div class="text-xs sm:text-sm mb-8">£ {{getJobPartHubzzFee(job_part.job.schedules) | currency}}</div>

				<div class="font-bold text-sm sm:text-md">Job Part Gross Rate</div>
				<div class="text-xs sm:text-sm mb-8">£ {{getJobPartGrossRate(job_part.schedules) | currency}}</div>
        ...
        <div
          class="text-xs sm:text-sm mb-8"
				...
				<div class="text-xs sm:text-sm mb-8 flex flex-row flex-wrap">
					<div v-if="job_part.job.platform_job.compliance_documents.length === 0" class="mt-1">(none)</div>
					<div
