    ...
    <div class="bg-white rounded-lg shadow-lg p-4 md:p-8 mt-4">
      <div class="flex flex-row flex-wrap">
        <div class="flex flex-col w-full lg:w-1/2 p-0 md:pr-4">
          ...
          <AppInput v-model="form.practice_id" :type="'select'" :name="'practice_id'" :label="'Practice'"
            :error="formError.find(item => item.field === 'practice_id')" :placeholder="'Select...'"
            :items="practice_lists" />
          <div class="text-xs sm:text-sm mt-2 mb-4 flex flex-row flex-wrap items-center">
            <div class="w-full">
              <AppInput v-model="form.rate" :type="'text'" :name="'rate'" :label="'Rate £'" :placeholder="''"
                :error="formError.find(item => item.field === 'rate')" />
            </div>
            <div class="w-full">
              <AppInput v-model="form.locum_detail_rate_type_id" :type="'select'" :name="'locum_detail_rate_type_id'"
                :label="'per'" :error="formError.find(item => item.field === 'locum_detail_rate_type_id')"
                :placeholder="'Select...'" :items="rate_lists" />
            </div>
          </div>
          <AppInput v-model="form.total_hours" :type="'text'" :name="'total_hours'" :label="'Total hours'"
            :placeholder="''" :error="formError.find(item => item.field === 'total_hours')" />
          <AppInput v-model="form.title" :type="'text'" :name="'title'" :label="'Title'" :placeholder="''"
            :error="formError.find(item => item.field === 'title')" />
          <AppInput v-model="form.description" :type="'textarea'" :name="'description'" :label="'Job description'"
            :error="formError.find(item => item.field === 'description')" :resize="false" />
          <AppInput v-model="form.report_to" :type="'text'" :name="'report_to'" :label="'Report to'" :placeholder="''"
          ...
          <AppInput v-model="form.phone_number" :type="'text'" :name="'phone_number'" :label="'Telephone number'"
            :placeholder="''" :error="formError.find(item => item.field === 'phone_number')" />
          <AppInput :type="'select'" v-model="form.is_another_doctor" :name="'is_another_doctor'"
          ...
          <AppInput :type="'select'" v-model="form.is_nurse_available" :name="'is_nurse_available'"
            :label="'Is nurse support available?'"
            :items="[ {value: 'true', label: 'YES'}, {value: 'false', label: 'NO'} ]"
            :error="formError.find(item => item.field === 'is_nurse_available')" />
          <AppInput v-model="form.number_of_patients" :type="'number'" :name="'number_of_patients'"
            :label="'Number of patients to be seen during the session?'" :inStyle="'text-align:right;'"
            :error="formError.find(item => item.field === 'number_of_patients')" />

          <AppInput v-model="form.duration_for_each_appointment" :type="'number'"
            :name="'duration_for_each_appointment'" :label="'Duration of each appointment?'"
            :inStyle="'text-align:right;'"
            :error="formError.find(item => item.field === 'duration_for_each_appointment')" />

          <AppInput :type="'select'" v-model="form.opportunity_for_catch_up_slots"
            :name="'opportunity_for_catch_up_slots'" :label="'Opportunity for catch up slots?'"
            :items="[ {value: true, label: 'YES'}, {value: false, label: 'NO'} ]"
            :error="formError.find(item => item.field === 'opportunity_for_catch_up_slots')" />
          <AppInput v-model="form.session_requirements" :type="'multi-checkbox'"
            ...
            :name="'session_requirements'" :label="'Session requirements'" :lists="session_requirements_lists"
            :error="formError.find(item => item.field === 'session_requirements')" />

          <AppInput v-model="form.session_structure_information" :type="'textarea'"
            :name="'session_structure_information'" :label="'Session structure information'"
            ...
            :error="formError.find(item => item.field === 'session_structure_information')" :resize="false" />
          ...
          <AppInput v-model="form.extra_information" :type="'textarea'" :name="'extra_information'"
            ...
            :error="formError.find(item => item.field === 'extra_information')" :resize="false" />

          <AppInput :type="'select'" v-model="session_amendment" :name="'session_amendment'"
            :label="'Choose one reason for session amendment?'" :items="session_amendment_list"
            :error="formError.find(item => item.field === 'session_amendment')" />
          <AppInput v-if="session_amendment === 'other'" v-model="form.update_remarks" :type="'textarea'"
            :name="'update_remarks'" :label="'Update Remarks'"
            :error="formError.find(item => item.field === 'update_remarks')" :resize="false" />
          <template v-if="['Allocated','Applied'].includes(job.status)">
            <AppInput v-model="form.update_accepted_until" :type="'select'" :name="'update_accepted_until'"
              :label="'Set deadline for appointed Locum to accept these changes (per hour)'"
              :error="formError.find(item => item.field === 'update_accepted_until')" :items="[
        ...
        <div class="flex flex-col w-full lg:w-1/2 p-0 md:pl-4">
          ...
          <div class="flex flex-row flex-wrap justify-between">
            <div class="px-1 w-full md:w-1/2">
              <AppDate v-model="form.date_start" :name="'date_start'" :label="'Start Date'"
                :error="formError.find(item => item.field === 'date_start')" isAfter />
            </div>
            <div class="px-1 w-full md:w-1/2">
              <AppTime v-model="form.time_start" :type="'time'" :name="'time_start'" :label="'Start Time'"
                :error="formError.find(item => item.field === 'time_start')" />
            </div>
            <div class="px-1 w-full md:w-1/2">
              <AppDate v-model="form.date_end" :name="'date_end'" :label="'End Date'"
                :error="formError.find(item => item.field === 'date_end')" :startDate="form.date_start" isAfter />
            </div>
            <div class="px-1 w-full md:w-1/2">
              <AppTime v-model="form.time_end" :type="'time'" :name="'time_end'" :label="'End Time'"
                :error="formError.find(item => item.field === 'time_end')" />
            </div>
            <div class="w-full">
              <AppInput v-if="show_saturday" :type="'select'" v-model="form.include_saturday" :name="'include_saturday'"
                :label="'Include Saturday'" :items="[{ label: 'Yes', value: true }, { label: 'No', value: false }]" />
              <AppInput v-if="show_sunday" :type="'select'" v-model="form.include_sunday" :name="'include_sunday'"
                :label="'Include Sunday'" :items="[{ label: 'Yes', value: true }, { label: 'No', value: false }]" />
            </div>
          </div>
          <AppInput v-model="form.shift_id" :type="'select'" :name="'shift_id'" :label="'Shift'"
            :placeholder="'Select...'" :items="shifts" :error="formError.find(item => item.field === 'shift_id')" />
          <AppInput v-model="unpaid_breaks" :type="'select'" :name="'unpaid_breaks '" :label="'Unpaid break'"
            :items="[ {value: false, label: 'No'}, {value: 15, label: '15'}, {value: 30, label: '30'}, {value: 60, label: '60'}, {value: 'other', label: 'Other'} ]" />
          <AppInput v-if="unpaid_breaks === 'other'" v-model="form.unpaid_breaks_in_minutes" :type="'text'"
            :name="'unpaid_breaks_in_minutes'" :label="'Other'" :inStyle="'text-align:right;'"
            :error="formError.find(item => item.field === 'unpaid_breaks_in_minutes')" />

          <AppInput :type="'select'" v-model="auto_assign_job" :name="'auto_assign_job'"
            :label="'Use AUTO-MATCH on this Job?'" :items="[ {value: false, label: 'No'}, {value: true, label: 'Yes'} ]"
          ...
          <template v-if="['false', false].includes(auto_assign_job)">
            <AppInput :type="'select'" v-model="selection_notification" :name="'selection_notification'"
              :label="'Add a selection date?'" :items="[ {value: false, label: 'No'}, {value: true, label: 'Yes'} ]"
              :inStyle="job.status === 'Allocated' ? 'background-color:lightgray' : ''"
              :disabled="job.status === 'Allocated'" />
            <div class="flex flex-row flex-wrap justify-between"
              ...
              <div class="px-1 w-full md:w-1/2">
                <AppTime v-model="selection_date.time" :type="'time'" :name="'time_end'" :label="'Time'"
          ...
          <AppInput :type="'select'" v-model="bank_only" :name="'bank_only'"
          ...
          <template v-if="['false', false].includes(bank_only)">
            <AppInput :type="'select'" v-model="bank_first" :name="'bank_first'"
              :label="'Make this Job available for Bank First?'"
              :items="[ {value: false, label: 'No'}, {value: true, label: 'Yes'} ]"
              :inStyle="job.status === 'Allocated' ? 'background-color:lightgray' : ''"
              :disabled="job.status === 'Allocated'" />
            <div class="flex flex-row flex-wrap justify-between" v-if="bank_first === true || bank_first === 'true'">
              <div>Only favorite locum will be notified until this date</div>
              <div class="px-1 w-full md:w-1/2">
                <AppDate v-model="favorite_only_until.date" :name="'favorite_only_until'" :label="'Date'"
                  :inStyle="job.status === 'Allocated' ? 'background-color:lightgray' : ''"
                  :disabled="job.status === 'Allocated'" />
              </div>
              <div class="px-1 w-full md:w-1/2">
                <AppTime v-model="favorite_only_until.time" :type="'time'" :name="'time_end'" :label="'Time'"
          ...
          <template v-if="parseInt(selectedProfessionCategoryId) === 1">
            <AppInput v-model="form.ir35" :type="'select'" :name="'ir35'"
          ...
          <AppInput v-model="form.profession_id" :type="'select'" :name="'profession_id'" :label="'Role'"
            :items="professions" :inStyle="job.status === 'Allocated' ? 'background-color:lightgray' : ''"
            :disabled="job.status === 'Allocated'" />

          <AppFilterSearch v-model="form.qualification_id" :name="'qualification_id'" :label="'Specialty'"
            :placeholder="'Select...'" :error="formError.find(item => item.field === 'qualification_id')"
            :info="'Choose at least one qualification'" :url="'/api/v1/qualifications'"
          ...
          <AppFilterSearch v-model="form.clinical_system_id" :name="'clinical_system_id'" :label="'Clinical systems'"
            ...
            :info="'Choose at least one IT system'" :url="'/api/v1/clinical-systems'"
          ...
          <AppFilterSearch v-model="form.spoken_language_id" :name="'spoken_language_id'" :label="'Spoken languages'"
            :placeholder="'Select...'" :info="'Choose other languages you can speak'" :url="'/api/v1/spoken-languages'"
            :defaultItem="'English'" :inStyle="job.status === 'Allocated' ? 'background-color:lightgray' : ''"
            :disabled="job.status === 'Allocated'" />

          <div class="font-bold text-sm sm:text-md">Compliance requirements</div>
          <AppInput v-model="form.compliance_document_id" :type="'multi-checkbox'"
            ...
            :placeholder="''" :lists="compliances" :disabled="job.status === 'Allocated'" />
          <div class="font-bold text-sm sm:text-md">Mandatory trainings</div>
          <AppInput v-model="form.mandatory_training_id" :type="'multi-checkbox'"
            @checked="form.mandatory_training_id.push($event)" @unchecked="uncheckMandatory($event)"
            :name="'mandatory_training_id'" :label="'Mandatory training required for this job'"
            :placeholder="'Select..'" :lists="mandatory_training_lists" :info="'Check all that apply'" />
          <div class="my-3" v-if="mandatory_training_lists.length === 0">
            <AppButton :label="'Go to Profile to add items here'" @click="goToProfile" :inStyle="'padding:4px;'" />
          </div>
        </div>
      </div>
      <div class="mb-8">
        <AppButton :label="'Save changes'" :inStyle="'padding:8px'" @click="validateUpdate" :disabled="loading" />
      </div>
    </div>
    <AppConfirmationModal
      :label="'You might lose some of your candidates that are not qualified to the changes you made'"
      :confirmLabel="'Proceed'" :cancelLabel="'Cancel'" :modal="modal" @confirm="save" @cancel="modal = false" />
  </div>
</template>
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (this.job.platform_job.session_requirements === "") {
        ...
        this.form.session_requirements = this.job.platform_job.session_requirements ? this.job.platform_job.session_requirements.split(
          ","
        ) : [];
      }
        ...
        if (!this.formError.length) {
          ...
          this.$axios
            ...
            .catch(err => {
              ...
              this.form.session_requirements = this.form.session_requirements ? this.form.session_requirements.split(
                ","
              ) : [];
