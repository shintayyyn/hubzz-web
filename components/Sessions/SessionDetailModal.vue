  ...
  <div class="p-4 md:p-8">
    ...
    <template v-if="['Allocated'].includes(job.status)">
      <div
        ...
        class="mt-4"
      >
        Waiting for locum's approval within {{ deadline.hours > 0 ? deadline.hours > 1 ? `${deadline.hours} hours` : `${deadline.hours} hour` : '' }} {{ deadline.hours ? 'and' : '' }} {{ deadline.minutes > 0 ? deadline.minutes > 1 ? `${deadline.minutes} minutes` : `${deadline.minutes} minute` : '' }}, before {{ $moment(updateDeadline).utc().format("h:mm a, MMMM D, YYYY") }}.
      </div>
    </template>
