				...
				<div class="flex flex-row justify-between">
					...
					<transition name="fade" mode="out-in">
						<div v-if="sendMessageModal" class="message-modal md:w-2/3 lg:w-1/2 xl:w-1/3">
							<SendMessageModal :user="user" @close="sendMessageModal=false" @showProfile="show(user.id)" />
						</div>
					</transition>
			...
			this.$axios
				...
				.then(res => {
					...
					this.mandatoryTrainings =
						res.data.user.locum_detail.mandatory_trainings;
		...
		getLocumCompliancesByLocumProfessionProfessionComplianceCategoryId(
			...
			this.$axios
				...
				.then(res => {
					...
					console.log("documents", this.mandatory);
					console.log("optional documents", this.optional);
