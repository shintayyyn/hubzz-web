<template>
  <section>
    <div class="p-4 md:p-8 max-w-3xl">
      ...
      <div v-if="user.locum_job_applied_update_accepted_until && user.locum_job_applied_update_accepted == false">
        <p class="text-sm">
          This user has not yet accepted your changes.
        </p>
        Waiting for locum's approval until
        {{ $moment(user.locum_job_applied_update_accepted_until).format("h:mm a, MMMM D, YYYY") }}
      </div>
      <div class="flex flex-row flex-wrap justify-between mt-4">
        ...
        <div class="w-full pl-0 lg:pl-2 mt-8 lg:mt-0 lg:w-1/2">
          ...
          <AppButton v-if="authPermissions.includes('Appoint Sessions Job')" :label="'Appoint to this job'"
                     :disabled="!user.locum_job_user_appointable" @click="confirmation_modal = true"
          />
          <!-- <AppButton
