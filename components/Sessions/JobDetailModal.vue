<template>
	<div class="p-4 md:p-8">
		...
		<div class="flex flex-wrap items-center justify-start mt-4 md:mt-8">
			<div class="flex items-center leading-loose font-bold text-md sm:text-lg">
				{{ job.title }}
				<div class="mx-2 text-sm sm:text-sm p-2" :class="bgStatus(job.status)">
					{{ status(job.status) }}
				</div>
			</div>
			<div v-if="authPermissions.includes('Update Sessions Job')">
				<button
					class="font-bold text-xs sm:text-sm no-underline px-2 py-2 rounded-lg bg-yellow-500 focus:outline-none"
					v-if="
		...
		<div class="flex flex-col mt-4">
			<div class="flex flex-wrap justify-start">
				...
				<div
					class="p-0 md:pl-4 w-full lg:w-1/2 mt-4 md:m-0 order-first lg:order-none"
				>
					<div class="flex flex-col">
