    ...
    <div class="wrapper p-4 md:p-8" ref="modalContainer">
      ...
      <div class="flex flex-row flex-wrap justify-start mt-8">
        <div class="w-full md:w-1/2 md:pr-4 mb-4">
          <div class="flex flex-col">
            ...
            <div class="bg-white rounded-lg shadow-lg px-4 md:px-8 py-4 mt-4">
              <AppInput v-model="form.practice_id" :type="'select'" :name="'practice_id'" :placeholder="'Select...'"
                :error="formError.find(item => item.field === 'practice_id')" :items="practice_lists"
                @blur="CheckEmptyField(form.practice_id, 'practice_id')" />
            </div>
            <div class="flex flex-col">
              <h4 class="font-bold mt-4">Criteria</h4>
              <div class="bg-white rounded-lg shadow-lg px-4 md:px-8 py-4 mt-4">
                <AppInput v-model="form.role" :type="'select'" :name="'role'" :label="'Role'" :placeholder="'Select...'"
                  :items="professions" :error="formError.find(item => item.field === 'role')"
                  @blur="CheckEmptyField(form.role,'role')" />

                <template v-if="form.role">
                  <AppFilterSearch v-model="form.specialty" :name="'specialty'" :label="'Specialty'"
                    :placeholder="'Select...'" :info="'Choose at least one qualification'"
                  ...
                  <AppFilterSearch v-model="form.clinical_system" :name="'clinical_system'" :label="'Clinical systems'"
                    :placeholder="'Select...'" :info="'Choose at least one IT system'" :url="'/api/v1/clinical-systems'"
                    :error="formError.find(item => item.field === 'clinical_system')" />

                  <AppFilterSearch v-model="form.spoken_language_id" :name="'spoken_language_id'"
                    :label="'Spoken languages'" :placeholder="'Select...'"
                    :info="'Choose other languages you can speak'" :url="'/api/v1/spoken-languages'"
                    :defaultItem="'English'" />
                  ...
                  <template v-if="form.role">
                    ...
                    <AppInput v-if="compliances.length > 0" v-model="form.compliance_document_id"
                      :type="'multi-checkbox'" :error="formError.find(item => item.field === 'compliance_document_id')"
            ...
            <div class="flex flex-col">
              ...
              <div class="bg-white rounded-lg shadow-lg px-4 md:px-8 py-4 mt-4">
                <div class="flex flex-row flex-wrap justify-between">
                  <div class="px-1 w-full md:w-1/2">
                    <AppDate v-model="form.date_start" :name="'date_start'" :label="'Start Date'"
                      ...
                      @blur="CheckEmptyField(form.date_start,'date_start')" isAfter />
                  </div>
                  <div class="px-1 w-full md:w-1/2">
                  ...
                  <div class="px-1 w-full md:w-1/2">
                    <AppDate v-model="form.date_end" :name="'date_end'" :label="'End Date'"
                      ...
                      @blur="CheckEmptyField(form.date_end,'date_end')" :startDate="form.date_start" isAfter />
                  </div>
                  <div class="px-1 w-full md:w-1/2">
                ...
                <AppInput v-if="show_saturday" :type="'select'" v-model="form.include_saturday"
                  :name="'include_saturday'" :label="'Include Saturday'"
                ...
                <AppInput :type="'select'" v-model="unpaid_breaks" :name="'unpaid_breaks'" :label="'Unpaid break'"
                ...
                <AppInput v-if="unpaid_breaks === 'other'" v-model="form.unpaid_breaks_in_minutes" :type="'text'"
                  :name="'unpaid_breaks_in_minutes'" :label="'Other'" :placeholder="''" :inStyle="'text-align:right;'"
                ...
                <AppInput v-model="form.shift" :type="'select'" :name="'shift'" :label="'Shifts'"
                  :placeholder="'Select...'" :items="shifts" :error="formError.find(item => item.field === 'shift')"
                ...
                <template v-if="['false', false].includes(bank_only)">
                  ...
                  <div class="flex flex-row flex-wrap justify-between"
                    ...
                    <div class="px-1 w-full md:w-1/2">
                      <AppDate v-model="favorite_only_until.date" :name="'favorite_only_until'" :label="'Date'" isAfter
        ...
        <div class="w-full md:w-1/2 lg:pl-4 mb-4">
          ...
          <div class="bg-white rounded-lg shadow-lg px-4 md:px-8 py-4 mt-4">
            <AppInput v-model="form.title" :type="'text'" :name="'title'" :label="'Title'"
              :error="formError.find(item => item.field === 'title')" @blur="CheckEmptyField(form.title,'title')" />

            <AppInput v-model="form.description" :type="'textarea'" :name="'description'" :label="'Description'"
              :resize="false" :error="formError.find(item => item.field === 'description')"
            ...
            <AppInput v-model="form.email" :type="'text'" :name="'email'" :label="'Email'" :placeholder="''"
              :error="formError.find(item => item.field === 'email')" @blur="CheckEmptyField(form.email,'email')" />
            ...
            <AppInput v-model="form.number_of_patients" :type="'number'" :name="'number_of_patients'"
              :label="'Number of patients to be seen during the session?'" :placeholder="''"
              :inStyle="'text-align:right;'" :error="formError.find(item => item.field === 'number_of_patients')"
            ...
            <AppInput v-model="form.duration_for_each_appointment" :type="'number'"
              :name="'duration_for_each_appointment'" :label="'Duration of each appointment?'" :placeholder="''"
            ...
            <AppInput v-model="form.session_requirements" :type="'multi-checkbox'"
              ...
              :name="'session_requirements'" :label="'Session requirements'" :placeholder="''"
              :lists="session_requirements_lists" :error="formError.find(item => item.field === 'session_requirements')"
            ...
            <AppInput v-model="form.session_structure_information" :type="'textarea'"
              ...
              :resize="false" :error="formError.find(item => item.field === 'session_structure_information')"
            ...
            <AppInput v-model="form.extra_information" :type="'textarea'" :name="'extra_information'"
              ...
              :placeholder="'For example, number of expected patients, nearby car park, etc.'" :resize="false" />

            <div class="flex flex-row flex-wrap justify-start items-center mt-4 max-w-2xl">
              <div class="px-1 w-full">
                <AppInput v-model="form.rate" :type="'number'" :name="'rate'" :label="'Rate £'" :min="1"
                  :error="formError.find(item => item.field === 'rate')" @blur="CheckEmptyField(form.rate,'rate')"
                  :inStyle="'text-align:right'" />
              </div>
              <div class="px-1 w-full">
                <AppInput v-model="form.locum_detail_rate_type_id" :type="'select'" :name="'locum_detail_rate_type_id'"
                  :label="'per'" :items="rate_lists" />
              </div>
            </div>
            <div class="flex flex-col py-2 mb-3 md:mb-6">
              ...
              <div class="flex flex-row flex-no-wrap justify-start mt-1">
                <div class="flex flex-col">
                  <input v-model="form.total_hours" type="number"
                    ...
                    @blur="CheckEmptyField(form.total_hours,'total_hours')" style="text-align:right;'"
                    :error="formError.find(item => item.field === 'total_hours')" @keypress="isNumber($event)" />
                  <div class="text-red-500 p-1 text-xs"
                    v-if="this.formError.find(item => item.field === 'total_hours')">
                    {{this.formError.find(item => item.field === 'total_hours').message.charAt(0).toUpperCase() + this.formError.find(item => item.field === 'total_hours').message.slice(1).replace(/_/g, " ")}}
                  </div>
                </div>
        ...
        <div class="pt-4 pb-8 w-full flex">
          <AppButton class="ml-auto" :label="'Save and publish Job'" @click="publish"
            v-if="authPermissions.includes('Create Sessions Job')" :disabled="loading" />
        </div>
      </div>
          ...
          ([
            ...
            if (this.repostJob) {
              ...
              if (this.repostJob.platform_job.session_requirements === "") {
                ...
                this.form.session_requirements = this.repostJob.platform_job.session_requirements ? this.repostJob.platform_job.session_requirements.split(
                  ","
                ) : [];
              }
