    ...
    <tbody>
      <template v-for="(item, index) in jobs">
        ...
        >
          ...
          <td>{{item.date_end}}</td>
          <td
            v-if="['Available','Applied','Current','Unfilled'].includes(item.status) || ['Current','Available','Applied','Matched'].includes(item.locum_status)"
          >{{$moment(item.date_created).format('YYYY-MM-DD') }}</td>
          <td v-if="item.status === 'Applied' || item.locum_status === 'Applied'">Date Applied</td>
          <template v-if="item.status === 'Current' || item.locum_status === 'Current'">
            <td v-if="item.type === 'Private'">N/A</td>
            <td v-else>{{item.platform_job.appointed_at | localDate}}</td>
          </template>
          <td
            v-if="item.status === 'Completed' || item.locum_status === 'Completed'"
          >{{item.job_parts[item.job_parts.length - 1].completed_at | localDate }}</td>
          <td
            v-if="item.status === 'Cancelled' || item.locum_status === 'Cancelled'"
          >{{item.platform_job.cancelled_at | localDate}}</td>
          <td
