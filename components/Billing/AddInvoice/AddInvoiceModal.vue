    ...
    <div class="absolute pin-r pin-t z-10 p-10 bg-grey-darkest" style="height:130%" :style="$store.state.mobile ? 'width:100%' : 'width:70%'">
        <div class="flex flex-row flex-no-wrap justify-start">
            ...
            <button class="border border-solid border-yellow-dark bg-yellow-dark text-black font-bold p-2 rounded-lg mx-2 hover:text-yellow hover:bg-grey-darkest" 
                @click.prevent="save_changes">Save changes</button>
            <button class="border border-solid border-yellow-dark bg-yellow-dark text-black font-bold p-2 rounded-lg mx-2 hover:text-yellow hover:bg-grey-darkest" 
                @click.prevent="archive_as_file">Save and archive as file</button>
        </div>
        ...
        <div class="border-solid bg-white mt-10 mb-5 pt-20 px-5 pb-5" style="width:70%">
            ...
            <div class="flex justify-start mt-5" >
                <div class="mb-4 border border-solid border-black px-5 pt-5 pb-8 rounded-lg" style="width:50%">
            ...
            <div class="flex justify-start mt-20 text-sm">
                Select a job to add to this invoice
            </div>

            <div class="flex justify-start mt-5">
                <div class="bg-black text-sm text-center text-white font-bold" style="width:60%;font-family:Nunito">Description</div>
                <div class="bg-black text-sm text-center text-white font-bold" style="width:35%;font-family:Nunito">Total</div>
            </div>

            <transition-group name="list">
                <JobInvoiceForm v-for="(item) in form.jobs" :key="item.id"
                    :id="item.id"
            ...
            <div class="flex justify-end mt-2" >
                <div style="width:5%" class="px-1">
                    <div class="rounded-full bg-black h-6 w-6 flex items-center justify-center text-white font-bold text-xl cursor-pointer" @click.prevent="add">+</div>
                </div>
            </div>

            <div class="flex justify-start mt-5">
              <div class="mr-1" style="width:30%;">
                <div class="mb-4 border-b border-b-2 border-grey py-2" :class="{'border-yellow-dark':setFocus === 'days_work_from'}">
                  <label for="days_work_from" class="block text-sm mb-2">Days worked from</label>
                  <input @focus="setFocus = 'days_work_from'" @blur="setFocus = ''" v-model="form.from" type="date" name="days_work_from" id="days_work_from"
              ...
              <div cclass="" style="width:30%;">
                <div class="mb-4 border-b border-b-2 border-grey py-2" :class="{'border-yellow-dark':setFocus === 'days_work_to'}">
                  <label for="days_work_to" class="block text-sm mb-2">To</label>
                  <input @focus="setFocus = 'days_work_to'" @blur="setFocus = ''" v-model="form.to" type="date" name="days_work_to" id="days_work_to"
                    class="appearance-none bg-transparent border-none w-full text-grey-darker mr-3 py-1 px-2 leading-tight focus:outline-none">
                </div>
              </div>
            </div>
