<template>
  <div class="p-8 max-w-2xl">
    ...
    <div class="flex flex-col mt-4">
      ...
      <div class="rounded-lg shadow-lg p-8 mt-4 w-full lg:w-2/3">
...
export default {
  ...
  methods: {
    ...
    getJobApplicants() {
      this.$axios.$get(`/api/v1/practice/jobs/${this.$store.state.session.job_id}/applicants`).then(res => {
        this.applicants = res.data.users
      })
    },
    show(id) {
      this.$store.commit('session/SET_USER_ID', id)
      this.$store.commit('SET_APPLIEDLOCUMDETAIL_MODAL', true)
      this.$store.commit('SET_APPLIEDLOCUMDETAIL_SHIELD', true)
