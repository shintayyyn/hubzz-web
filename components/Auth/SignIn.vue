        ...
        <div class="rounded-lg shadow-lg px-10 py-12" >
            <form class="w-full">
                <div class="relative flex flex-col mt-8 border-b-2 border-white"
                    ...
                    <input type="email" name="email" ref="email" id="email" class="focus:outline-none font-bold" style="height:40px"
                ...
                <div class="relative flex flex-col mt-8 border-b-2 border-white"
                    ...
                    <input type="password" name="password" id="password" class="focus:outline-none font-bold" style="height:40px"
                        @focus="setFocus = 'password'" @blur="setFocus = ''" v-model="form.password" @keyup.enter="login">
                    <span class="absolute pin-r bg-red text-white p-1" v-if="formError.find(item => item.field === 'password')">
                        {{formError.find(item => item.field === 'password').message}}
                    </span>
                </div>
            </form>
            <div class="mt-8 flex justify-end">
                <span class="text-right cursor-pointer hover:underline" @click="$store.commit('SET_ACTIVE_TAB', 'forgot_password')">Forgot password ?</span>
            </div>
            <div class="mt-5 flex justify-center">
                <button class="rounded-lg bg-yellow-dark shadow-md p-8 font-bold text-xl focus:outline-none hover:text-white" @click="login">Sign In</button>
            </div>
        </div>
    </section>
</template>
