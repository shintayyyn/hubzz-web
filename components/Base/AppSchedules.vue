							...
							<div class="px-4">
								<div class="flex items-end text-sm pb-2 font-bold text-gray-700">
									...
									>
										...
										<!-- FOR COMPLETING JOB -->
										<p v-if="['complete', 'terminate', 'invoice'].includes(type)" class="w-2/12">Final Start</p>

										<p v-if="['complete', 'terminate', 'invoice'].includes(type)" class="w-2/12">Final End</p>
										...
										<!-- FOR INVOICING -->
										<p v-if="type === 'invoice'" class="w-2/12">Final Hours</p>

										<p v-if="type === 'invoice'" class="w-2/12">Final Rate</p>

										<p v-if="type === 'invoice'" class="w-2/12">Dispute?</p>
					...
					>
						...
						>
							...
							<div v-if="['complete', 'terminate'].includes(type)" class="flex justify-between">
								<p
									class="w-2/3"
								>Job Part {{ job_part_queue_number }}/{{ jobPartTerminationCompletion.job.job_parts.length }} Total Hours:</p>
							...
							<div v-if="['complete', 'terminate'].includes(type)" class="flex justify-between">
								<p
									class="w-2/3"
								>Job Part {{ job_part_queue_number }}/{{ jobPartTerminationCompletion.job.job_parts.length }} Gross Rate:</p>
							...
							>
								<p
									class="w-2/3"
								>Job Part {{ job_part_queue_number }}/{{ jobPartTerminationCompletion.job.job_parts.length }} Tax Rate:</p>

								<p
									class="w-1/3"
								>£ {{ getJobTaxRate(filteredSchedule, ['complete', 'terminate'].includes(type)) | currency }}</p>
							</div>
							...
							>
								<p
									class="w-2/3"
								>Job Part {{ job_part_queue_number }}/{{ jobPartTerminationCompletion.job.job_parts.length }} Taxed Gross Rate:</p>

								<p
									class="w-1/3"
								>£ {{ getJobTaxedGrossRate(filteredSchedule, ['complete', 'terminate'].includes(type)) | currency }}</p>
							</div>

							<div
								v-if="['complete', 'terminate'].includes(type)"
								class="flex justify-between border-t-2 text-gray-500"
							>
								<p class="w-2/3">Total Job Gross Rate(Tentative):</p>

								<p
									class="w-1/3"
								>£ {{ getEntireJobGrossRate(jobPartTerminationCompletion.job.schedules) | currency }}</p>
							</div>
