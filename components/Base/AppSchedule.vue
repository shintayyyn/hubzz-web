<template>
	<div>
		<transition name="fade">
			<div
				v-if="showShifts"
				class="fixed m-auto z-50 left-0 right-0 top-0 bottom-0 err-shield flex items-center justify-center"
			>
				<div
					class="overflow-hidden relative bg-white border rounded mx-2 md:mx-0"
					:class="shift_schedule.length > 0 ? 'md:w-4/5 xl:w-3/5' : 'md:w-1/4'"
				>
					...
					<div class="flex items-center justify-between px-4 pt-2 pb-2 border-b">
						...
						<svgicon
							...
							width="18"
							height="18"
							@click="(shift_schedule && shift_schedule.find(item => item.initial) || editShiftId) ? [confirmClosing = true, confirmation={message: 'Discard Changes?', type: 'close'}] : showShifts=false, shiftError=[]"
							class="fill-current hover:text-gray-700 cursor-pointer"
						/>
					</div>
					<template v-if="!forCompletion">
						<div class="px-4">
							...
							<transition name="fade">
								<div
									:class="shift_schedule.length > 1 ? 'overflow-x-hidden overflow-y-auto':''"
									style="max-height: 600px"
									ref="scheduleWrapper"
									v-if="shift_schedule.length > 0"
								>
									<!-- SAVED SHIFTS -->
									<div
										class="relative py-3"
										:class="[index%2?'bg-gray-200':'', shift_schedule.length >= 4 ? 'px-1 lg:pl-0 lg:pr-2' : 'px-1', index !== (shift_schedule.length-1) ? 'border-b-2 ' : '']"
										v-for="(item, index) in shift_schedule"
										:key="index"
									>
										<transition name="fade">
											<div
												class="absolute h-full w-full flex justify-center items-center z-20"
												v-if="shift_saving===index"
											>
												<p class="bg-green-400 px-8 py-3 rounded-lg font-bold flex items-center text-white">
													<span>
														<svgicon name="success-checkmark" width="20" height="20" class="mr-1 fill-current" />
													</span>Saved!
												</p>
											</div>
										</transition>
										<template
											v-if="!item.initial && (!editShiftId || (editShiftId && editShiftId !== item.value))"
										>
											<div class="flex justify-between items-center border-b mb-2 pb-2">
												<p class="font-bold">
													<span class="font-normal">Shift Name:</span>
													{{ item.label }}
												</p>
												<div class="flex text-sm">
													<p
														class="hover:text-blue-600 cursor-pointer mr-2"
														@click="editShift('edit', item)"
													>Edit</p>
													<p
														class="hover:text-blue-600 cursor-pointer mr-2"
														@click="[confirmClosing = true, confirmation={message: 'Are you sure you want to remove this shift?', type: 'remove', data: {id:item.id, index: index}}]"
													>Remove</p>
													<p
														class="cursor-pointer"
														:class="item.appliedToAll ? 'text-red-500 hover:text-red-600' : 'hover:text-blue-600 '"
														@click="applyToAll(item)"
													>{{ item.appliedToAll ? 'Cancel Apply to All' : 'Apply to All' }}</p>
												</div>
											</div>
											<div class="flex flex-col">
												...
												<div class="flex font-semibold">
													<p
														class="w-1/5"
													>{{item.shift ? item.shift.name : shifts.find(shift => shift.value.toString() === item.shift_id.toString()).label}}</p>
													<p class="w-1/5">{{item.time_start}}</p>
													<p class="w-1/5">{{item.time_end}}</p>
													<p
														class="w-2/5"
													>{{item.rate}} {{item.locum_detail_rate_type ? item.locum_detail_rate_type.name : rate_lists.find(rate => rate.value.toString() === item.locum_detail_rate_type_id.toString()).label}}</p>
												</div>
											</div>
										</template>
										<template v-else>
											...
											<div class="flex flex-col md:flex-row md:justify-end">
												<AppButton
													v-if="editShiftId && editShiftId === item.value"
													:label="'Cancel Editing'"
													class="md:mr-1 mb-1 md:mb-0"
													@click="cancelEditing(item, index)"
												/>
												<AppButton
													v-if="!item.initial"
													:label="editShiftId && editShiftId === item.value ? 'Save' : 'Edit'"
													class="md:mr-1 mb-1 md:mb-0"
													@click="editShift('save', item, index)"
												/>
												<AppButton
													v-if="item.initial"
													:label="'Save'"
													class="md:mr-1 mb-1 md:mb-0"
													@click="saveShiftSchedule(item, index)"
												/>
												<AppButton
													v-if="item.initial"
													:label="'Remove'"
					...
					<div class="flex items-center justify-end border-t p-4">
						<p
							v-if="shift_schedule && shift_schedule.length"
							class="text-gray-700 font-bold text-xs mx-2 px-2 flex items-center justify-center border-b-2"
						>{{ shift_schedule.length }}</p>
						<AppButton
							v-if="shift_schedule && shift_schedule.length"
							:label="'Add Shift'"
							:inStyle="'padding:5px 14px;'"
						...
						<AppButton
							:label="'Close'"
							:inStyle="'padding:5px 14px;'"
							@click="(shift_schedule && shift_schedule.find(item => item.initial) || editShiftId) ? [confirmClosing = true, confirmation={message: 'Discard Changes?', type: 'close'}] : closeShift('closeButton')"
						/>
					</div>
				</div>
			</div>
	...
	created() {
		...
		if (this.jobSchedule) {
			...
			this.jobSchedule.forEach((sched, i) => {
				let shift_id = this.shift_schedule.findIndex(
					shift => shift.name === sched.schedule_template_name
				);
				let isExistingDate = this.schedule.find(
					item => item.date === sched.date
				);
				if (isExistingDate) {
	...
	watch: {
		schedule_dates(value) {
			if (value.length) {
				...
				if (!removedAnItem) {
					let isExisting = this.schedule.find(
