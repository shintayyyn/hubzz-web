...
export default {
	...
	computed: {
		job() {
			return this.isPrivate ? this.propJob.job : this.propJob;
		},
		isJobPart() {
			return (
		...
		isPrivate() {
			return (this.propJob.job && this.propJob.job.private_job) ||
				(this.propJob.job &&
					this.propJob.job.locum_status.toLowerCase() === "ongoing")
				? true
				: false;
		},
		// isNotUnavailable () {
		...
		// },
		bgStatus() {
			let job = this.isPrivate ? this.propJob.job : this.propJob;
