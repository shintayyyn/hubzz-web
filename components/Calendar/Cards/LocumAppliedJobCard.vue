<template>
  <div
    ...
    @click="select"
  >
    <div class="bg-orange-light absolute pin-l pin-t rounded-l-lg p-2 h-full"></div>
    <div class="ml-2">
      ...
      >{{job.platform_job.practice.surgery.address.line_1}}, {{job.platform_job.practice.surgery.address.post_code}}</div>
      <div
        class="text-grey-dark my-3 text-xs xl:text-sm"
      >From {{job.platform_job.date_start}} to {{job.platform_job.date_end}}</div>
      <div class="text-grey-dark my-3 text-xs xl:text-sm">Shift: {{job.platform_job.shift.name}}</div>
      <div class="my-3 text-xs xl:text-sm">{{job.platform_job.description}}</div>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    select() {
      ...
      this.$axios.$get(`/api/v1/locum/jobs/${this.job.id}`).then(res => {
        this.$emit('viewLocumAppliedJob', res.data.job)
      })
    }
  }
}
