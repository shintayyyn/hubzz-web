    ...
    getCalendarJobAndJobParts(calendarDateStart, calendarDateEnd) {
      if (
        ...
        Promise.all([
          ...
          .then(([jobParts, jobs]) => {
            const allocatedJobParts = jobParts.filter(
              ({ status }) => status === "Allocated"
            );
            const withdrawnJobParts = jobParts.filter(
            ...
            const unfilledJobs = jobs.filter(
        ...
        Promise.all([
          ...
          .then(
            ([
              jobParts,
              jobs,
              permanentJobApplications,
              locumUnavailabilities
            ]) => {
              const allocatedJobParts = jobParts.filter(
                ({ status, type }) =>
                  status === "Allocated" && type === "Platform"
              );
              const privateJobParts = jobParts.filter(
                ({ type }) => type === "Private"
              );
              ...
              this.locumUnavailabilities = locumUnavailabilities.filter(
                locumUnavailability => locumUnavailability.shifts.length > 0
              );
            }
          )
          .finally(() => {
