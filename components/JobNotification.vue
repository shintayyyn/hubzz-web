		...
		<transition-group name="drop" mode="out-in">
				...
				>
					...
					<div class="flex flex-wrap mt-3 w-48 md:w-64">
							...
							>
								{{
									notification.status
										...
										: notification.locum_status.toUpperCase()
								}}
							</div>
						</div>
						<div class="flex justify-between items-center my-1 w-full">
						...
						<div class="flex justify-between items-center my-1 w-full">
							...
							<div
								v-text="
									`£ ${notification.rate} ${notification.locum_detail_rate_type}`
								"
							></div>
						</div>
						<div class="flex justify-between items-center my-1 w-full">
			...
			<template v-for="billingNotification in billingNotifications">
				<nuxt-link
					...
					:key="
						`${billingNotification.id}-${billingNotification.notification_type}`
					"
				>
					<div
						class="relative m-1 p-3 flex flex-wrap bg-gray-100 rounded-lg shadow-lg"
						...
						<div class="flex flex-wrap mt-3 w-48 md:w-64">
							<div class="flex justify-between items-center my-1 w-full">
								<div class="font-bold text-lg">
									{{ billingNotification.invoice_number }}
								</div>
								<div class="mx-1"></div>
								<div
									class="px-2 py-1 text-sm font-bold rounded-lg max-w-sm cursor-pointer"
								...
								>
									{{ billingNotification.status.toUpperCase() }}
								</div>
							</div>
							<div class="flex justify-between items-center my-1 w-full">
								<div>From</div>
								<div class="text-right">
									{{ billingNotification.date_start }}
								</div>
							</div>
							<div class="flex justify-between items-center my-1 w-full">
								<div>To</div>
							...
							<div class="flex justify-between items-center my-1 w-full">
								<div>Issued At</div>
								<div class="text-right">
									{{ billingNotification.issued_at | localDate }}
								</div>
							</div>
							<div class="flex justify-between items-center my-1 w-full">
								<div>Paid At</div>
								<div class="text-right">
									{{ billingNotification.paid_at | localDate }}
								</div>
							</div>
							<div
							...
							>
								<div>Locum</div>
								<div class="text-right">
									{{ billingNotification.locum_user }}
								</div>
							</div>
							<div
								class="flex justify-between items-center my-1 w-full"
