<template>
  <div class="rounded-r-lg appointment-section p-2 sm:px-6 sm:py-8" style="height:100%">
    <div
      class="text-white text-xs sm:text-sm md:text-base lg:text-lg"
    >{{$store.state.dashboard.select_date ? $moment($store.state.dashboard.select_date).format('Do MMMM, YYYY') : $moment($store.state.dateToday).format('Do MMMM, YYYY')}}</div>
      ...
      <transition-group name="slide-info" mode="out-in">
        ...
        >
          <div
            ...
            :class="checkShift(appointmentDates.shifts[0])"
          ></div>
          <div class="ml-2">
