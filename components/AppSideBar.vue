  ...
  <section>
    ...
    >
      <div class="sidebar-nav pt-8 xl:pt-10">
        ...
        <div
          v-for="(navigationTab, index) in navigationTabs"
          :key="index"
          class="text-sm relative"
        >
          <span
            v-if="navigationTab.active"
            class="absolute ml-3 mt-4 bg-sunglow p-1.5 h-1.5 rounded-sm"
          />

          <nuxt-link
            :to="navigationTab.route"
            :event="
              $route.path.includes(navigationTab.route) ? 'click' : 'click'
            "
            class="block no-underline pl-4 py-3 mx-4 transition-hover"
            :class="
              navigationTab.active
                ? 'text-white font-bold'
                : 'text-gray-500 hover:text-white hover:font-bold'
            "
          >
    ...
    <div
      v-if="survey"
      class="shield"
      @click.prevent="(survey = null), (surveyResponseSubmitted = false)"
    />

    <div v-if="survey">
      <div
        v-if="!surveyResponseSubmitted"
        class="rounded-lg shadow-md px-4 py-8 md:px-8 survey-modal border w-5/6 md:w-1/2 overflow-scroll"
        style="max-height: 70vh;"
      >
        <div
          v-for="surveyResponseAnswer in surveyResponseAnswers"
          :key="surveyResponseAnswer.survey_question_id"
        >
          <div
            v-if="surveyResponseAnswer.question_type === 'Text'"
            class="mb-4"
          >
            <h1 class="text-lg">
            ...
            <input
              v-model="surveyResponseAnswer.answer"
              class="w-full p-2 border rounded-lg bg-yellow-500 shadow-lg"
              type="text"
            >
          </div>

          <div
            v-if="surveyResponseAnswer.question_type === 'Text Box'"
            class="mb-4"
          >
            <h1 class="text-lg">
              {{ surveyResponseAnswer.question }}
          ...
          <div
            v-if="surveyResponseAnswer.question_type === 'Rating'"
            class="mb-4"
          >
            <h1 class="text-lg">
              {{ surveyResponseAnswer.question }}
            ...
            <div class="flex flex-wrap">
              <span
                ...
                :class="
                  `${
                    (surveyResponseAnswer.answer || 0) >= number
                      ? 'bg-yellow-500'
                      : 'bg-gray-200'
                  }`
                "
          ...
          <div
            v-if="surveyResponseAnswer.question_type === 'Check Boxes'"
            class="mb-4"
          >
            <h1 class="text-lg">
              {{ surveyResponseAnswer.question }}
            ...
            <div class="flex flex-col ml-2">
              ...
              >
                <AppInput
                  :value="
                    surveyResponseAnswer.selected_choices.includes(choice.id)
                  "
                  ...
                  @input="
                    selected =>
                      toggleSelectedChoice(
                        selected,
                        surveyResponseAnswer.survey_question_id,
                        choice.id
                      )
                  "
                />
          ...
          <div
            v-if="surveyResponseAnswer.question_type === 'Radio Buttons'"
            class="mb-4"
          >
            <h1 class="text-lg">
              {{ surveyResponseAnswer.question }}
...
export default {
    ...
    navigationTabs() {
      ...
      if (domain === "Locum") {
        ...
        locumTabList.push({
          ...
          active:
            `/${this.$route.path.split("/")[1]}` === "/terms-and-conditions"
        });
      ...
      if (domain === "Practice") {
        ...
        if (
          ...
          practiceTabList.push({
            ...
            active:
              `/${this.$route.path.split("/")[1]}` === "/hub-surgery-management"
          });
        }
        ...
        if (
          ...
          practiceTabList.push({
            ...
            route: "/my-banks",
            active: ["/my-banks", "/my-banks-reports"].includes(
              `/${this.$route.path.split("/")[1]}`
            )
          });
        }
        ...
        if (
          ...
          (["Hub", "Stand Alone"].includes(practiceType) ||
            (practiceType === "Spoke" && parentPracticeId !== null)) &&
          hubType !== "Type 2" &&
          this.authPermissions.includes("View My Banks") === false &&
          this.authPermissions.includes("View Practice Reports")
        ) {
          practiceTabList.push({
        ...
        if (
          ...
          (["Hub", "Stand Alone"].includes(practiceType) ||
            (practiceType === "Spoke" && parentPracticeId !== null)) &&
          this.authPermissions.includes("View Sessions Job") === false &&
          this.authPermissions.includes("View Practice Reports") &&
          hubType !== "Type 2"
        ) {
          practiceTabList.push({
            ...
            active:
              `/${this.$route.path.split("/")[1]}` === "/practice-job-reports"
          });
        }
        ...
        if (
          ...
          practiceTabList.push({
            ...
            active:
              `/${this.$route.path.split("/")[1]}` === "/practice-billing" ||
        ...
        if (
          ...
          (["Hub", "Stand Alone"].includes(practiceType) ||
            (practiceType === "Spoke" && parentPracticeId !== null)) &&
          this.authPermissions.includes("View Billings") === false &&
          this.authPermissions.includes("View Practice Reports") &&
          hubType !== "Type 2"
        ) {
          practiceTabList.push({
        ...
        if (
          ...
          practiceTabList.push({
            ...
            active:
              `/${this.$route.path.split("/")[1]}` === "/roles-and-permissions"
          });
        }
  ...
  mounted() {
    ...
    this.$socket.on(
      "User Notification Account Deleted",
      this.accountDeletedHandler
    );
    ...
    this.$socket.on(
      ...
      this.practiceDeletedHandler
    );

    this.$socket.on(
      "Practice Notification Practice User Deleted",
      this.practiceUserDeletedHandler
    );

    if (
      this.$auth.user &&
      this.$auth.user.domain === "Locum" &&
      this.$auth.user.status === "Deactivated"
    ) {
      this.locumAccountDeactivatedHandler();
    }
    ...
    if (
      this.$auth.user &&
      this.$auth.user.domain === "Practice" &&
      this.$auth.user.practice_status === "Deactivated"
    ) {
      this.practiceDeactivatedHandler();
    }
  },
  ...
  methods: {
    toggleSelectedChoice(selected, surveyQuestionId, choiceId) {
      const index = this.surveyResponseAnswers.findIndex(
      ...
      if (index > -1) {
        ...
        if (selected) {
          ...
          surveyResponseAnswer.selected_choices = surveyResponseAnswer.selected_choices.filter(
