<template>
  <section class="relative">
    ...
    <div v-if="getLocumUnsuccessfulJobs.length > 0" class="overflow-x-auto overflow-y-hidden p-0 md:p-2">
      <JobTable :columns="columns" :jobs="getLocumUnsuccessfulJobs" @sortBy="sortBy" @show="show" />
    </div>
    <div class="w-full mt-4" v-if="getLocumUnsuccessfulJobs.length > 0 && totalPages > 1">
      <AppPagination
        :total="total"
...
export default {
  ...
  methods: {
    ...
    show(id) {
      this.$router.push(`/jobs/unsuccessful/${id}`);
    }
  }
};
</script>