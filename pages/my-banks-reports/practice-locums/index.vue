...
export default {
    ...
    itemCountInfo() {
      ...
      const lastItem = Math.min(
        this.limit * this.activePage -
          this.limit +
          (this.loading ? this.limit : this.locumUsedReports.length),
        this.count
      );
  ...
  methods: {
    ...
    getLocumUsedReports() {
      ...
      Promise.all([
        this.$axios
          .get("/api/v1/practice/locum-used-reports/count", {
            cache: true,
            params
          })
          .then(responses => {
        ...
        this.$axios
          .get("/api/v1/practice/locum-used-reports", {
            cache: true,
            params: {
              ...params,
              order_by: this.orderBy,
    ...
    async downloadCsv() {
      ...
      try {
        const response = await this.$axios.post(
          "/api/v1/practice/locum-used-reports/generate-key",
        ...
        window.open(
          `${process.env.API_URL}/api/v1/locum-used-reports/csv?token=${response.data.data.token}`
        );
      } catch (err) {
        console.error(err);
      } finally {
