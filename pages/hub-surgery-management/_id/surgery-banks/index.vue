    ...
    <div class="flex flex-row flex-wrap justify-start">
      ...
      >
        ...
        >
          <nuxt-link
            :to="{
              path: `/hub-surgery-management/${$route.params.id}/surgery-banks/${locum.id}`,
              query: { ...$route.query }
            }"
          >
            <div class="flex flex-wrap text-center mt-4 cursor-pointer">
              <div class="w-full flex justify-center">
                <AppAvatar
                  :src="
                    locum.avatar && locum.avatar.file && locum.avatar.file.url
      ...
      <transition name="fade" mode="out-in">
        <nuxt-link
          v-if="
            ['hub-surgery-management-id-surgery-banks-locumId'].includes(
              $route.name
            )
          "
          class="shield"
...
export default {
  ...
  methods: {
    getLocumsCount() {
      ...
      this.loading = true;
      this.$axios
        .$get(`/api/v1/practice/locums/count`, { params, cache: true })
        .then(res => {
    ...
    getLocums(page) {
      ...
      this.$axios
        .$get(`/api/v1/practice/locums`, { params, cache: true })
        .then(res => {
