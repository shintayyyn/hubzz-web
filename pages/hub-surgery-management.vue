<template>
  <section>
    <AppBreadcrumbs
      v-if="
        ![
          'hub-surgery-management-index',
          'hub-surgery-management-invitations-spoke',
          'hub-surgery-management-invitations-hub'
        ].includes($route.name)
      "
      :links="links"
    />
    <div
      v-if="
        !$route.params.id &&
          ...
          !$route.params.jobId &&
          !['hub-surgery-management-invitations-hub-create'].includes(
            $route.name
          )
      "
    ...
    >
      <nuxt-link
        ...
        :class="
          $route.name === 'hub-surgery-management-index' ||
      ...
      <nuxt-link
        v-if="
          authPermissions &&
            authPermissions.includes('Invitation Processes Surgery Management')
        "
    ...
    links() {
      ...
      case "hub-surgery-management-id-surgery-billings-invoices-from-locums":
        links.push(
          ...
          {
            title: `${
              this.$route.query.status === "pension-form-a" ? "NHS " : ""
            }${
              this.$route.query.status
                ? this.$route.query.status
                  .replace(/-/g, " ")
                  .replace(/(^\w{1})|(\s{1}\w{1})/g, word =>
                    word.toUpperCase()
                  )
                : "To be invoiced"
            }`,
      ...
      if (this.$route.params.permJobId) {
        links.push({
          ...
          url: `/hub-surgery-management/${
            this.$route.params.id
          }/surgery-permanent-jobs/${this.$route.params.permJobId}${
            this.$route.query.status
              ? "?status=" + this.$route.query.status
              : ""
          }`
