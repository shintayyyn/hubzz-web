					...
					<div v-if="form.vat_registered === true" class="flex flex-col items-start mb-4">
						...
						<div class="flex items-center">
							...
							<p
								...
								class="text-sm mx-2"
							>{{ form.vat_certificate.name }}</p>
						</div>
						<transition name="drop-down">
							<div
								v-if="formError.find(item => item.field === 'vat_certificate')"
								class="text-red-500 py-1 text-xs text-white"
							>{{ formError.find(item => item.field === 'vat_certificate').message.charAt(0).toUpperCase() + formError.find(item => item.field === 'vat_certificate').message.slice(1).replace(/_/g, " ") }}</div>
						</transition>
					</div>
						...
						<div class="flex flex-col items-start mb-4">
							...
							<div class="flex items-center">
								...
								<p
									...
									class="text-sm mx-2"
								>{{ form.certificate_of_incorporation.name }}</p>
							</div>
							<transition name="drop-down">
								<div
									v-if="formError.find(item => item.field === 'certificate_of_incorporation')"
									class="text-red-500 py-1 text-xs text-white"
								>{{ formError.find(item => item.field === 'certificate_of_incorporation').message.charAt(0).toUpperCase() + formError.find(item => item.field === 'certificate_of_incorporation').message.slice(1).replace(/_/g, " ") }}</div>
							</transition>
						</div>
					</template>
		...
		<div class="w-full lg:w-auto mb-4 lg:mb-0 p-0 lg:pr-4 order-1 lg:order-2">
			<div class="rounded-lg shadow-lg w-full py-8 px-12">
				...
				<div class="leading-none text-center text-sm pt-4">
					<p class="font-bold">{{ profile ? profile.name : '' }}</p>
