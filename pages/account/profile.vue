<template>
  <div class="flex flex-col lg:flex-row justify-start">
      ...
      <div class="relative w-full">
        ...
        <form v-if="!loading && user" class="w-full flex flex-col md:flex-row items-start">
          <div class="w-full md:w-1/2 rounded-lg border p-4 mr-4">
            ...
            <AppInput v-if="false || (user.profession && user.profession.name !== 'GP')" v-model="form.profession_id"
                      :type="'select'" :name="'profession_id'" :label="'Profession'" :placeholder="'Select...'"
                      :items="professionsSelectionList" required :disabled="false"
            />

            <AppFilterSearch v-if="selectedProfession && selectedProfession.sub_professionable"
                             v-model="selectedSubProfessionsSelectionList" label="Sub Professions" placeholder="Select..."
                             :error="formError.find(item => item.field === 'sub_profession_ids')"
                             info="Choose other professions if applicable" :selectionLists="subProfessionsSelectionList"
            />

            <div v-for="referenceLocumComplianceDocument in referenceLocumComplianceDocuments"
                 :key="referenceLocumComplianceDocument.compliance_document_id"
            >
              <AppInput v-model="referenceLocumComplianceDocument.reference" type="text"
                        ...
                        :limit="getReferenceLimit(referenceLocumComplianceDocument.compliance_document_name)" required
            ...
            <AppInput :value="form.ni_number" :type="'text'" :name="'ni_number'" :label="'NI number'"
                      :error="formError.find(item => item.field === 'ni_number')" :placeholder="'AA000000A'" :limit="9" required
                      @input="(v) => form.ni_number = v.toUpperCase()" @keydown="(e) => mixinNiNumber(e, form.ni_number)"
            />

            <AppFilterSearch v-model="form.qualification_id" :name="'qualification_id'" :label="'Speciality'"
                             :placeholder="'Select...'" :error="formError.find(item => item.field === 'qualification_id')"
                             :info="'Choose at least one qualification'" :url="'/api/v1/qualifications'" :addNaOption="true"
                             :showClearButton="true" :defaultNaIfEmpty="true"
                             :professionCategoryId="!professionCategoryId ? null : professionCategoryId.toString()" required
            />

            <AppFilterSearch v-model="form.clinical_system_id" :name="'clinical_system_id'" :label="'Clinical systems'"
                             :placeholder="'Select...'" :error="formError.find(item => item.field === 'clinical_system_id')"
                             :info="'Choose at least one IT system'" :url="'/api/v1/clinical-systems'" required
            ...
            <AppFilterSearch v-model="form.spoken_language_id" :name="'spoken_language_id'" :label="'Spoken languages'"
                             :placeholder="'Select...'" :info="'Choose other languages you can speak'"
                             :url="'/api/v1/spoken-languages'" :defaultItem="'English'"
            />

            <AppInput v-model="form.nhs_smart_card_id_number" :type="'text'" :name="'nhs_smart_card_id_number'"
                      :label="'Your NHS Smart Card ID number'"
                      :error="formError.find(item => item.field === 'nhs_smart_card_id_number')" :limit="12"
                      @keydown="(e) => mixinNumber(e)" @submit="updateLocumProfile"
            />

            <AppInput v-model="form.headline" :type="'text'" :name="'headline'" :label="'Headline'"
                      :info="'A short headline about yourself'" @submit="updateLocumProfile"
            />

            <AppInput v-model="form.short_biography" :type="'textarea'" :name="'short_biography'" :label="'Biography'"
                      :info="'A short headline about yourself'" :resize="false" @submit="updateLocumProfile"
            />

            <AppInput v-model="form.special_requirements" :type="'textarea'" :name="'special_requirements'"
                      :label="'Special requirements'" :info="'Information for Practices for your own needs'" :resize="false"
                      @submit="updateLocumProfile"
            />

            <AppInput v-model="form.vat_registered" :type="'single-checkbox'" :name="'vat_registered'"
                      :label="'Are you VAT Registered?'" :error="formError.find(item => item.field === 'vat_registered')"
            />

            <AppInput v-if="form.vat_registered" v-model="form.vat_number" :type="'numberDash'" :name="'vat_number'"
                      :label="'VAT Number'" :error="formError.find(item => item.field === 'vat_number')" :limit="11"
                      @submit="updateLocumProfile" @keydown="(e) => mixinNumber(e)"
            />
                ...
                <div class="flex flex-row justify-start mt-1">
                  <div class="flex flex-col w-full">
                    ...
                    <transition name="drop-down">
                      <div v-if="formError.find(item => item.field === 'vat_number')"
            ...
            <div v-if="form.vat_registered === true" class="flex flex-col items-start mb-4">
              ...
              <div class="flex items-center">
                ...
                >
                  <input id="file" type="file" class="inputfile hidden" accept="image/jpeg, .pdf., .docx, .msword"
                         @input="onVATFileInput($event)" @click.stop
                  >
                ...
                <p v-if="form.vat_certificate && form.vat_certificate.name" class="text-sm mx-2">
                  <a :href="user.vat_cert_file_url" :download="user.vat_cert_file_filename" target="_blank"
                     class="truncate"
              ...
              <transition name="drop-down">
                <div v-if="formError.find(item => item.field === 'vat_certificate')"
                     class="text-red-500 py-1 text-xs text-white"
                >
                  {{ formError.find(item => item.field === 'vat_certificate').message.charAt(0).toUpperCase() +
                    formError.find(item => item.field === 'vat_certificate').message.slice(1).replace(/_/g, " ") }}
                </div>
              </transition>
            </div>

            <AppInput v-model="form.employment_type" :type="'select'" :name="'employment_type'" :label="'Are you...?'"
                      :items="employmentTypes" required
            />

            <template v-if="form.employment_type === 'Limited Company'">
              <AppInput v-model="form.company_registration_number" :type="'text'" :name="'company_registration_number'"
                        :label="'Company Registration Number'"
                        :error="formError.find(item => item.field === 'company_registration_number')"
                        :placeholder="'The number of your company from Companies House'" required
              ...
              <div class="flex flex-col items-start mb-4">
                ...
                <div class="flex items-center">
                  <div
                    class="flex flex-row flex-no-wrap justify-center items-center px-4 py-1 my-2 text-sm rounded bg-yellow-500 cursor-pointer hover:underline"
                  >
                    <input id="fileInc" type="file" class="inputfile hidden" accept="image/jpeg, .pdf., .docx, .msword"
                           @input="onIncFileInput($event)" @click.stop
                    >
                  ...
                  " class="text-sm mx-2"
                  >
                    <a :href="user.cert_of_incorp_file_url" :download="user.cert_of_incorp_file_filename"
                ...
                <transition name="drop-down">
                  <div v-if="formError.find(item => item.field === 'certificate_of_incorporation')"
                       class="text-red-500 py-1 text-xs text-white"
                  >
                    {{ formError.find(item => item.field ===
                      'certificate_of_incorporation').message.charAt(0).toUpperCase() + formError.find(item => item.field
                      === 'certificate_of_incorporation').message.slice(1).replace(/_/g, " ") }}
                  </div>
                </transition>
              </div>
            </template>
            ...
            <template v-if="form.employment_type === 'Self-Employed'">
              <AppInput v-model="form.utr_number" :type="'text'" :name="'utr_number'" :label="'UTR number'"
                        :error="formError.find(item => item.field === 'utr_number')" :placeholder="'0000000000'" :limit="10"
                        required @keydown="alphaNumeric($event)"
              />
            </template>

            <AppInput v-model="form.paid_under_payroll" :type="'single-checkbox'" :name="'paid_under_payroll'"
                      :label="'Are you paid under payroll?'"
                      :items="[{ label: 'Yes', value: true }, { label: 'No', value: false }]" required
            />
            ...
            <template v-if="form.paid_under_payroll == true || form.paid_under_payroll == 'true'">
              ...
              <AppInput v-model="form.payroll_account_name" :type="'text'" :name="'payroll_account_name'"
                        :label="'Payroll Company Name'" :error="formError.find(item => item.field === 'payroll_account_name')"
                        required
              />

              <AppInput v-model="form.payroll_company_email" :type="'text'" :name="'payroll_company_email'"
                        :label="'Payroll Company Email Address'"
                        :error="formError.find(item => item.field === 'payroll_company_email')" required
              />

              <AppInput v-model="form.payroll_bank_name" :type="'text'" :name="'payroll_bank_name'" :label="'Bank Name'"
                        :error="formError.find(item => item.field === 'payroll_bank_name')" required
              />

              <AppInput v-model="form.payroll_sort_code" :type="'numberDash'" :name="'payroll_sort_code'"
                        :label="'Sort Code'" :error="formError.find(item => item.field === 'payroll_sort_code')" :limit="8"
                        required @keydown="mixinSortCode($event)"
              />

              <AppInput v-model="form.payroll_account_number" :type="'number'" :name="'payroll_account_number'"
                        :label="'Payroll Bank Account Number'"
                        :error="formError.find(item => item.field === 'payroll_account_number')" :limit="8" required
                        @keydown="mixinAccountNumber($event)"
              />

              <AppInput v-model="form.payroll_reference_number" :type="'number'" :name="'payroll_reference_number'"
                        :label="'Payroll Reference Number'"
                        :error="formError.find(item => item.field === 'payroll_reference_number')" required
              />
            </template>
            ...
            <template v-if="form.paid_under_payroll == false || form.paid_under_payroll == 'false'">
              ...
              <AppInput v-model="form.sort_code" :type="'numberDash'" :name="'sort_code'" :label="'Sort Code'"
                        :error="formError.find(item => item.field === 'sort_code')" :limit="8" required
                        @keydown="mixinSortCode($event)"
              />

              <AppInput v-model="form.account_number" :type="'text'" :name="'account_number'" :label="'Account Number'"
                        :error="formError.find(item => item.field === 'account_number')" :limit="8" required
                        @keydown="mixinAccountNumber($event)"
              />
            </template>

            <template v-if="professionCategoryId === 1">
              <AppInput v-model="form.ir35" :type="'single-checkbox'" :name="'ir35'"
                        :label="'Are you willing to work for a role captured within IR35 rules, subject to deduction of Tax and N.I.?'"
              />
            </template>

            <template v-if="professionCategoryId === 1">
              <AppInput v-model="form.claim_nhs" :type="'single-checkbox'" :name="'claim_nhs'"
                        :label="'Are you willing to claim NHS Pension contributions?'"
              />

              <template v-if="form.claim_nhs == true || form.claim_nhs == 'true'">
                <AppInput v-model="form.epc_percentage_rate" :type="'select'" :name="'epc_percentage_rate'"
                          :label="'Employee pension contribution rate'" :items="[
                            { label: '5.2%', value: 5.2 },
                            { label: '6.5%', value: 6.5 },
                            { label: '8.3%', value: 8.3 },
                            { label: '9.8%', value: 9.8 },
                ...
                <AppInput v-model="form.section_scheme_year" :type="'select'" :name="'section_scheme_year'"
                          :label="'NHS Pension Scheme Year?'" :items="schemeYearLists" required
                />

                <AppInput v-model="form.nhs_number" :type="'text'" :name="'nhs_number'"
                          :label="'NHS Pension Scheme membership (SD) number'"
                          :error="formError.find(item => item.field === 'nhs_number')" :limit="8" required
                          @keydown="(e) => mixinNumber(e)"
                />

                <AppInput v-model="form.ay_percentage_rate" :type="'number'" :name="'ay_percentage_rate'"
                          :label="'Added Years % Rate'" :error="formError.find(item => item.field === 'ay_percentage_rate')"
                />
                <AppInput v-model="form.mpavc_percentage_rate" :type="'number'" :name="'mpavc_percentage_rate'"
                          :label="'Money Purchase AVC % Rate'"
                          :error="formError.find(item => item.field === 'mpavc_percentage_rate')"
                />
                <AppInput v-model="form.apc_percentage_rate" :type="'number'" :name="'apc_percentage_rate'"
                          :label="'Additional Pension Contribution'"
                          :error="formError.find(item => item.field === 'apc_percentage_rate')"
                />
                <AppInput v-model="form.errbo_percentage_rate" :type="'number'" :name="'errbo_percentage_rate'"
                ...
                />-->
                <AppInput v-model="form.nhs_registration_number" :type="'text'" :name="'nhs_registration_number'"
            ...
            <AppPostCode v-model="form.post_code" :urlIndex="'/api/v1/postcode-coordinates'" :name="'post_code'"
                         :label="'The post code where I will be available at'"
                         :error="formError.find(item => item.field === 'post_code')" required
                         @blur="CheckEmptyField(form.post_code, 'post_code')"
            />

            <AppInput v-model="form.miles" :type="'number'" :name="'miles'"
                      :label="`I will travel up to ${form.miles ? form.miles : 0} miles from this postcode`"
                      :error="formError.find(item => item.field === 'miles')" :limit="8" required
          ...
          <div class="w-full md:w-1/2">
            <div class="rounded-lg border p-4 mb-4">
              ...
              <AppInput v-model="form.view_locum_jobs" :type="'single-checkbox'" :name="'view_locum_jobs'"
                        :label="'Hubzz Locum Jobs'" :error="formError.find(item => item.field === 'view_locum_jobs')"
              />

              <AppInput v-model="form.view_permanent_jobs" :type="'single-checkbox'" :name="'view_permanent_jobs'"
              ...
              <div v-if="form.view_permanent_jobs" class="px-4">
                ...
                <AppInput v-model="form.view_permanent_jobs_full_time" :type="'single-checkbox'"
                          :name="'view_permanent_jobs_full_time'" :label="'Full Time'"
                          :error="formError.find(item => item.field === 'view_permanent_jobs_full_time')"
                />

                <AppInput v-model="form.view_permanent_jobs_part_time" :type="'single-checkbox'"
                          :name="'view_permanent_jobs_part_time'" :label="'Part Time'"
              ...
              <div class="flex flex-col my-6">
                ...
                <div class="flex flex-col md:flex-row md:items-end justify-between">
                  <div class="flex flex-col w-full md:w-1/3 md:pr-2">
                    <AppInput v-model="form.min_rate_per_hour" :type="'number'" :name="'min_rate_per_hour'"
                              :label="'Per Hour £'" :error="formError.find(item => item.field === 'min_rate_per_hour')" required
                              @submit="updateLocumProfile"
                  ...
                  <div class="flex flex-col w-full md:w-1/3 md:px-2">
                    <AppInput v-model="form.min_rate_per_half_day_session" :type="'number'"
                              :name="'min_rate_per_half_day_session'" :label="'Per Half Day Session £'"
                              :error="formError.find(item => item.field === 'min_rate_per_half_day_session')" required
                              @submit="updateLocumProfile"
                  ...
                  <div class="flex flex-col w-full md:w-1/3 md:pl-2">
                    <AppInput v-model="form.min_rate_per_whole_day_session" :type="'number'"
                              :name="'min_rate_per_whole_day_session'" :label="'Per Whole Day Session £'"
                              :error="formError.find(item => item.field === 'min_rate_per_whole_day_session')" required
                              @submit="updateLocumProfile"
              ...
              <AppInput v-model="form.mandatory_training_id" :type="'multi-checkbox'" :name="'mandatory_training_id'"
                        :label="'Please select additional training courses completed.'" :lists="mandatoryTrainings"
              ...
              <AppInput v-model="form.other_mandatory_training_id" :type="'multi-checkbox'"
                        :name="'other_mandatory_training_id'" :label="'Other Additional Trainings courses you completed'"
                        :lists="otherMandatoryTrainings" updatable
                        ...
                        @uncheckAll="form.other_mandatory_training_id = []" @addList="addList" @updateList="updateList"
                        @remove="toggleRemoveMandatoryModal"
              />

              <AppInput v-model="form.practice_type_id" :type="'multi-checkbox'" :name="'practice_type_id'"
                        :label="'What type of Practice(s) would you like to work for?'"
                        :error="formError.find(item => item.field === 'practice_type_id')" :lists="practiceTypesSelectionList"
                        required :showSelectAll="true"
            ...
            <div class="rounded-lg bg-gray-400 p-4 my-4">
              <AppInput v-model="form.referee_1_contact_name" :type="'text'" :name="'referee_1_contact_name'"
                        :label="'Contact Name'" :inStyle="'background-color:#dae1e7;border-color:white'"
                        :error="formError.find(item => item.field === 'referee_1_contact_name')"
              />

              <AppInput v-model="form.referee_1_position" :type="'text'" :name="'referee_1_position'"
                        :label="'Position'" :inStyle="'background-color:#dae1e7;border-color:white'"
                        :error="formError.find(item => item.field === 'referee_1_position')"
              />

              <AppInput v-model="form.referee_1_phone_number" :type="'text'" :name="'referee_1_phone_number'"
                        :error="formError.find(item => item.field === 'referee_1_phone_number')" :label="'Telephone Number'"
                        :inStyle="'background-color:#dae1e7;border-color:white'" :limit="11"
                        @keydown="(e) => mixinPhoneNumber(e)"
              />

              <AppInput v-model="form.referee_1_email" :type="'text'" :name="'referee_1_email'" :label="'Email Address'"
            ...
            <div class="rounded-lg bg-gray-400 p-4 my-4">
              <AppInput v-model="form.referee_2_contact_name" :type="'text'" :name="'referee_2_contact_name'"
                        :label="'Contact Name'" :inStyle="'background-color:#dae1e7;border-color:white'"
                        :error="formError.find(item => item.field === 'referee_2_contact_name')"
              />

              <AppInput v-model="form.referee_2_position" :type="'text'" :name="'referee_2_position'"
                        :label="'Position'" :inStyle="'background-color:#dae1e7;border-color:white'"
                        :error="formError.find(item => item.field === 'referee_2_position')"
              />

              <AppInput v-model="form.referee_2_phone_number" :type="'text'" :name="'referee_2_phone_number'"
                        :error="formError.find(item => item.field === 'referee_2_phone_number')" :label="'Telephone Number'"
                        :inStyle="'background-color:#dae1e7;border-color:white'" :limit="11"
                        @keydown="(e) => mixinPhoneNumber(e)"
              />

              <AppInput v-model="form.referee_2_email" :type="'text'" :name="'referee_2_email'" :label="'Email Address'"
                        :inStyle="'background-color:#dae1e7;border-color:white'"
    ...
    <AppConfirmationModal :label="'Proceed to remove this mandatory training?'" :confirmLabel="'Yes'"
                          :cancelLabel="'Cancel'" :modal="toggle_remove_mandatory_modal" @confirm="removeMandatory"
...
export default {
  ...
  mounted() {
    ...
    Promise.all([
      ...
      this.$axios
        .get("/api/v1/practice-types?limit=1000000")
      ...
      await this.$axios
        ...
        .then(res => {
          let index = this.otherMandatoryTrainings.findIndex(
            item => item.value === res.data.locum_other_mandatory_training.id
          )
