...
export default {
  ...
  created() {
    ...
    // 
    this.form.qualification_id = this.user.locum_detail.qualifications.map((qualification) => {
      return { label: qualification.name, value: qualification.id }
    })
    this.form.clinical_system_id = this.user.locum_detail.clinical_systems.map((clinicalSystem) => {
      return { label: clinicalSystem.name, value: clinicalSystem.id }
    })
    this.form.spoken_language_id = this.user.locum_detail.spoken_languages.map((spokenLanguage) => {
      return { label: spokenLanguage.name, value: spokenLanguage.id }
    })
    // rate
    this.form.min_rate_per_hour = this.user.locum_detail.rates.find(rate => rate.rate_type.id === 1).min
    this.form.max_rate_per_hour = this.user.locum_detail.rates.find(rate => rate.rate_type.id === 1).max
    this.form.min_rate_per_half_day_session = this.user.locum_detail.rates.find(rate => rate.rate_type.id === 2).min
    this.form.max_rate_per_half_day_session = this.user.locum_detail.rates.find(rate => rate.rate_type.id === 2).max
    this.form.min_rate_per_whole_day_session = this.user.locum_detail.rates.find(rate => rate.rate_type.id === 3).min
    this.form.max_rate_per_whole_day_session = this.user.locum_detail.rates.find(rate => rate.rate_type.id === 3).max
    //  practice type
    this.form.practice_type_id = this.user.locum_detail.practice_types.map(practiceType => practiceType.id)
    // mandatory trainings list
    this.form.mandatory_training_id = this.user.locum_detail.mandatory_trainings.map(mandatoryTraining => mandatoryTraining.mandatory_training.id)
    // 
      ...
      try {
        ...
        if (!this.formError.length) {
          ...
          this.form.spoken_language_id = this.form.spoken_language_id
            ...
            : [];
          const response = await this.$axios.$put(`/api/v1/locum/me/profile`, this.form)
          this.form.gmc_or_nmc_number = response.data.user.locum_detail.gmc_or_nmc_number.number
          this.form.mpl_or_npl_number = response.data.user.locum_detail.mpl_or_npl_number.number
          this.form.nhs_smart_card_id_number = response.data.user.locum_detail.nhs_smart_card_id_number
          this.form.headline = response.data.user.locum_detail.headline
          this.form.short_biography = response.data.user.locum_detail.short_biography
          this.form.special_requirements = response.data.user.locum_detail.special_requirements
          // profession
          this.form.profession_id = response.data.user.locum_detail.profession.id
          // 
          this.form.qualification_id = response.data.user.locum_detail.qualifications.map((qualification) => {
            return { label: qualification.name, value: qualification.id }
          })
          this.form.clinical_system_id = response.data.user.locum_detail.clinical_systems.map((clinicalSystem) => {
            return { label: clinicalSystem.name, value: clinicalSystem.id }
          })
          this.form.spoken_language_id = response.data.user.locum_detail.spoken_languages.map((spokenLanguage) => {
            return { label: spokenLanguage.name, value: spokenLanguage.id }
          })
          // rate
          this.form.min_rate_per_hour = response.data.user.locum_detail.rates.find(rate => rate.rate_type.id === 1).min
          this.form.max_rate_per_hour = response.data.user.locum_detail.rates.find(rate => rate.rate_type.id === 1).max
          this.form.min_rate_per_half_day_session = response.data.user.locum_detail.rates.find(rate => rate.rate_type.id === 2).min
          this.form.max_rate_per_half_day_session = response.data.user.locum_detail.rates.find(rate => rate.rate_type.id === 2).max
          this.form.min_rate_per_whole_day_session = response.data.user.locum_detail.rates.find(rate => rate.rate_type.id === 3).min
          this.form.max_rate_per_whole_day_session = response.data.user.locum_detail.rates.find(rate => rate.rate_type.id === 3).max
          //  practice type
          this.form.practice_type_id = response.data.user.locum_detail.practice_types.map(practiceType => practiceType.id)
          // mandatory trainings list
          this.form.mandatory_training_id = response.data.user.locum_detail.mandatory_trainings.map(mandatoryTraining => mandatoryTraining.mandatory_training.id)
          // 
