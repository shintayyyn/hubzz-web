<template>
  <section class="relative">
    ...
    <AppJobFilter v-if="getPracticeDeclinedJobs.length > 0" @clear="clearFilters" @getJobs="getJobs(1, params)" :params="params" />
    <div
    ...
    <div
      class="shield"
      v-if="$route.name === 'sessions-declined-id'"
      @click="$router.push(`/sessions/declined`)"
    ></div>
    <nuxt-child />
  </section>
</template>
  ...
  methods: {
    ...
    show(id) {
      if (this.authPermissions.includes("Show Sessions Job")) {
        this.$router.push(`/sessions/declined/${id}`);
      }
    }
  }
};
