<template>
  <section class="relative">
    ...
    <AppJobFilter v-if="getPracticeCancelledJobs.length > 0" @clear="clearFilters" @getJobs="getJobs(1, params)" :params="params" />
    <div
    ...
    <div class="w-full mt-4" v-if="getPracticeCancelledJobs.length > 0 && totalPages > 1">
      <AppPagination
        :total="total"
    ...
    <div
      class="shield"
      v-if="$route.name === 'sessions-cancelled-id'"
      @click="$router.push(`/sessions/cancelled`)"
    ></div>
    <nuxt-child />
  </section>
</template>
  ...
  methods: {
    ...
    show(id) {
      if (this.authPermissions.includes("Show Sessions Job")) {
        this.$router.push(`/sessions/cancelled/${id}`);
      }
    }
  }
};
