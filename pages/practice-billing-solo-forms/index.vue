      ...
      <div v-if="!initialLoading">
          ...
          <template v-slot:actions="slotProps">
            <div class="flex flex-col bg-white border rounded border-gray-500">
              ...
              >
                ...
                <div
                  v-if="
                    !slotProps.item.practice_paid_at_formatted &&
                      authPermissions.includes('Process Billings')
                  "
                  class="rounded text-xs px-2 hover:bg-sunglow cursor-pointer "
                  @click="
                    locumSoloFormIdToMarkAsPaidByPractice = slotProps.item.id
                  "
                >
                  Mark as Paid
                </div>
        ...
        <div v-if="locumSoloFormIdToMarkAsPaidByPractice" class="p-2">
          ...
          >
            ...
            <AppDate
              ...
              :error="
                formErrors.find(
                  formError => formError.field === 'practice_paid_at'
                )
              "
              is-before
            />
        ...
        <transition name="fade" mode="out-in">
          <nuxt-link
            ...
            class="shield"
            @click.native="
              (locumSoloFormIdToBeSend = null), (locumSoloFormIdToSign = null)
            "
          />
        </transition>
    ...
    viewAsPdf(locumSoloFormId) {
      this.$axios
        ...
        .then((res) => {
          ...
          window.open(
            `${process.env.API_URL}/api/v1/locum-solo-form/${locumSoloFormId}/pdf?token=${token}`
          );
        })
        .catch((err) => {
          console.log("err", err);
