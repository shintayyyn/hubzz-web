<template>
	<div class="p-4">
		<div v-if="totalSiblings < 1" class="flex justify-center text-gray-500">You have no siblings.</div>
		<AppTable
		...
		>
			<template v-slot:actions="slotProps">
				<div class="flex flex-wrap justify-center">
					<div
						@click="$router.push({ path: `/spoke-surgery-management/spoke-siblings/${slotProps.item.id}` })"
		...
		<transition name="fade" mode="out-in">
			<nuxt-link
				...
				v-if="$route.name.includes('spoke-surgery-management-spoke-siblings-id')"
			></nuxt-link>
		</transition>
		<nuxt-child />
	</div>
