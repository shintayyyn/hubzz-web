<template>
  <div class="modal-container shadow-lg p-4 md:p-8">
    ...
    <div
      class="relative bg-white rounded-lg shadow-lg p-4 md:p-8 mt-4 max-w-5xl"
    >
      <AppInput
        v-model="search_text"
        type="text"
    ...
    >
      ...
      <div class="border-t-2 p-4 text-xs xl:text-base">
        ...
        <p class="font-bold">
          Try again with practice code or its full name if the practice isn't in
          the result.
        </p>
      </div>
    </div>
...
export default {
  components: { AppInput, AppButton, AppConfirmationModal },
  ...
  methods: {
    async search() {
      ...
      try {
        const invitationsRes = await this.$axios.$get(
          `/api/v1/practice/me/parent-surgery/invitations`,
          { params: { is_invitation_accepted: false } }
        );

        const invitationIds = invitationsRes.data.practice_surgeries.map(
          ps => ps.practice_id
        );

        const practicesRes = await this.$axios.$get(
          `/api/v1/practice/practice-spokes?search=${this.search_text}&limit=10`,
          { params: { practice_type: ["Hub", "Stand Alone"] } }
        );

        const practices = practicesRes.data?.practices ?? [];
        this.practiceHubs = practices.map(practice => ({
          ...practice,
        ...
        console.log(error);
        this.notify("danger", "Something went wrong!");
      }
    },
    ...
    cancelInvite() {
      ...
      this.selectedPractice = null; // ✅ Fix: clear stale selection on cancel
    },

    invite() {
      const { id, surgery } = this.selectedPractice;

      this.$axios
        .$post(`/api/v1/practice/me/parent-surgery/invite`, { practice_id: id })
        .then(res => {
          // ✅ Fix: mark the item as invited in-place so UI reflects immediately
          const match = this.practiceHubs.find(p => p.id === id);
          if (match) this.$set(match, "invited", true);
        ...
        .finally(() => {
          this.toggle_invite_modal = false;
          this.selectedPractice = null; // ✅ Fix: always clear after attempt
        });
    },
