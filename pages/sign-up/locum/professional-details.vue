        ...
        <div class="flex flex-col p-8 m-1 rounded-lg shadow-lg" style="flex: 0 1 600px;">
          ...
          <div class="flex flex-col mb-8">
            <div class="flex flex-wrap justify-between">
              ...
              <div class="m-1 flex-auto flex justify-end">
                <span class="text-xs p-1 bg-grey-lighter" v-if="!selectedQualificationsErrorMessage || showQualificationsSelect">Choose at least one qualification</span>
                <span class="text-xs bg-red p-1 text-white" v-if="selectedQualificationsErrorMessage && !showQualificationsSelect">{{ selectedQualificationsErrorMessage }}</span>
              </div>
            </div>
            <div class="relative" v-on-clickaway="hideQualificationsSelect">
              <div class="flex items-center flex-wrap border-b" :class="showQualificationsSelect ? 'border-yellow-dark' : selectedQualificationsErrorMessage ? 'border-red' : 'border-grey-dark'">
                <div class="px-2 py-1 rounded-lg m-1" style="background-color: #FFDA3A;" v-for="selectedQualification in selectedQualifications">
                  <span class="text-sm">{{ selectedQualification.name }} </span>
                  <button @click="removeQualification(selectedQualification)">x</button>
                </div>
                <div style="flex: 1 0 50%;">
                  <input class="w-full bg-grey-lighter outline-none px-1 py-2 mt-1" ref="inputQualificationsSearch" v-model="qualificationsSearch" @click="showQualificationsSelect = true" @keydown="inputQualificationSearchKeydownHandler" @focus="showQualificationsSelect = true" @blur="checkSelectedQualifications" placeholder="Select">
                </div>
              </div>
              <div class="bg-white absolute pin-l pin-r z-10 border shadow-lg rounded-lg overflow-auto" ref="qualificationsContainer" style="top: 100%; max-height: 175px;" v-if="showQualificationsSelect">
                <div v-for="(qualification, index) in filteredQualifications" :ref="`qualification-${index}`" class="p-2 cursor-pointer" :class="`${index !== qualifications.length ? 'border-b' : ''} ${index === qualificationIndex ? 'bg-grey-light' : ''}`" @click="addQualification(qualification)" @mouseover="qualificationIndex = index">
          ...
          <div class="flex flex-col mb-8">
            <div class="flex flex-wrap justify-between">
              ...
              <div class="m-1 flex-auto flex justify-end">
                <span class="text-xs p-1 bg-grey-lighter" v-if="!selectedSystemsErrorMessage || showSystemsSelect">Choose at least one IT system</span>
                <span class="text-xs bg-red p-1 text-white" v-if="selectedSystemsErrorMessage && !showSystemsSelect">{{ selectedSystemsErrorMessage }}</span>
              </div>
            </div>
            <div class="relative" v-on-clickaway="hideSystemSelect">
              <div class="flex items-center flex-wrap border-b" :class="showSystemsSelect ? 'border-yellow-dark' : selectedSystemsErrorMessage ? 'border-red' : 'border-grey-dark'">
                <div class="px-2 py-1 rounded-lg m-1" style="background-color: #FFDA3A;" v-for="selectedSystem in selectedSystems">
                  <span class="text-sm">{{ selectedSystem.name }} </span>
                  <button @click="removeSystem(selectedSystem)">x</button>
                </div>
                <div style="flex: 1 0 50%;">
                  <input class="w-full bg-grey-lighter outline-none px-1 py-2 mt-1" ref="inputSystemsSearch" v-model="systemsSearch" @click="showSystemsSelect = true" @keydown="inputSystemSearchKeydownHandler" @focus="showSystemsSelect = true" @blur="checkSelectedSystems" placeholder="Select">
                </div>
              </div>
              <div class="bg-white absolute pin-l pin-r z-10 border shadow-lg rounded-lg overflow-auto" ref="systemsContainer" style="top: 100%; max-height: 175px;" v-if="showSystemsSelect">
                <div v-for="(system, index) in filteredSystems" :ref="`system-${index}`" class="p-2 cursor-pointer" :class="`${index !== systems.length ? 'border-b' : ''} ${index === systemIndex ? 'bg-grey-light' : ''}`" @click="addSystem(system)" @mouseover="systemIndex = index">
          ...
          <div class="flex flex-col mb-8">
            <div class="flex flex-wrap justify-between">
              ...
              <div class="m-1 flex-auto flex justify-end">
                <span class="text-xs p-1 bg-grey-lighter" v-if="!selectedLanguagesErrorMessage || showLanguagesSelect">Choose other languages you can speak</span>
                <span class="text-xs bg-red p-1 text-white" v-if="selectedLanguagesErrorMessage && !showLanguagesSelect">{{ selectedLanguagesErrorMessage }}</span>
              </div>
            </div>
            <div class="relative" v-on-clickaway="hideLanguageSelect">
              <div class="flex items-center flex-wrap border-b" :class="showLanguagesSelect ? 'border-yellow-dark' : selectedLanguagesErrorMessage ? 'border-red' : 'border-grey-dark'">
                <div class="px-2 py-1 rounded-lg m-1" style="background-color: #FFDA3A;">
                  <span class="text-sm">English </span>
                </div>
                <div class="px-2 py-1 rounded-lg m-1" style="background-color: #FFDA3A;" v-for="selectedLanguage in selectedLanguages">
                  <span class="text-sm">{{ selectedLanguage.name }} </span>
                  <button @click="removeLanguage(selectedLanguage)">x</button>
                </div>
                <div style="flex: 1 0 50%;">
                  <input class="w-full bg-grey-lighter outline-none px-1 py-2 mt-1" ref="inputLanguagesSearch" v-model="languagesSearch" @click="showLanguagesSelect = true" @keydown="inputLanguageSearchKeydownHandler" @focus="showLanguagesSelect = true" @blur="checkSelectedLanguages" placeholder="Select">
                </div>
              </div>
              <div class="bg-white absolute pin-l pin-r z-10 border shadow-lg rounded-lg overflow-auto" ref="languagesContainer" style="top: 100%; max-height: 175px;" v-if="showLanguagesSelect">
                <div v-for="(language, index) in filteredLanguages" :ref="`language-${index}`" class="p-2 cursor-pointer" :class="`${index !== languages.length ? 'border-b' : ''} ${index === languageIndex ? 'bg-grey-light' : ''}`" @click="addLanguage(language)" @mouseover="languageIndex = index">
          ...
          <div class="flex flex-col mb-8">
            <div class="flex flex-wrap justify-between">
              ...
              <div class="m-1 flex-auto flex justify-end">
                <span class="text-xs p-1 bg-grey-lighter">To match available jobs with</span>
              </div>
            </div>
            <div class="flex flex-wrap">
              <div class="flex-1 flex flex-col mr-4">
                <div class="flex flex-wrap justify-between">
                  ...
                  <div class="m-1 flex-auto flex justify-end">
                    <span class="text-xs bg-red p-1 text-white" v-if="ratePerHourErrorMessage && !showRatePerHourFocus">{{ ratePerHourErrorMessage }}</span>
                  </div>
                </div>
                <input class="outline-none py-1 mt-1 border-b w-full text-right" :class="showRatePerHourFocus ? 'border-yellow-dark' : ratePerHourErrorMessage ? 'border-red' : 'border-grey-dark'" v-model="ratePerHour" @focus="showRatePerHourFocus = true" @blur="showRatePerHourFocus = false, checkRatePerHour()">
              </div>
              <div class="flex-1 flex flex-col">
                <div class="flex flex-wrap justify-between">
                  <label class="text-xs my-1 py-1">Per session</label>
                  <div class="m-1 flex-auto flex justify-end">
                    <span class="text-xs bg-red p-1 text-white" v-if="ratePerSessionErrorMessage && !showRatePerSessionFocus">{{ ratePerSessionErrorMessage }}</span>
                  </div>
                </div>
                <input class="outline-none py-1 mt-1 border-b w-full text-right" :class="showRatePerSessionFocus ? 'border-yellow-dark' : ratePerSessionErrorMessage ? 'border-red' : 'border-grey-dark'" v-model="ratePerSession" @focus="showRatePerSessionFocus = true" @blur="showRatePerSessionFocus = false, checkRatePerSession()">
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col mb-8">
            <div class="flex flex-wrap justify-between">
              <label class="text-xs my-1 py-1">Are you OK to work with Practices that are inside of scope for IR35?</label>
            </div>
            <select class="outline-none py-2 mt-1 border-b w-full bg-grey-lighter" :class="showScopeIR35Focus ? 'border-yellow-dark' : false ? 'border-red' : 'border-grey-dark'" v-model="scopeIR35" @focus="showScopeIR35Focus = true" @blur="showScopeIR35Focus = false">
