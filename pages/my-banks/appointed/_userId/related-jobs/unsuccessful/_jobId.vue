<template>
  <div class="modal-container shadow-lg">
    <JobDetailModal
      :job="job"
      @close="$router.push(`/my-banks/appointed/${$route.params.userId}/related-jobs/unsuccessful`)"
    />
  </div>
</template>
<script>
  ...
  async asyncData({ app, route, store, error }) {
    try {
      let jobRes = await app.$axios.get(
        `/api/v1/practice/jobs/${route.params.jobId}`
      );
      const job =
        jobRes.data && jobRes.data.data && jobRes.data.data.job
