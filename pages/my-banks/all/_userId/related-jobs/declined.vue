<template>
  <section class="__jobs-section">
    ...
    <div class="shield" v-if="$route.name === 'my-banks-all-userId-related-jobs-declined-jobId'"></div>
    <nuxt-child />
  </section>
</template>
<script>
...
export default {
  ...
  async asyncData({ app, params, error }) {
    try {
      ...
      // count
      const responseCount = await app.$axios.$get(`/api/v1/practice/jobs/count?locum_detail_id=${user.locum_detail.id}&locum_status=Declined`)
      const total = responseCount.data && responseCount.data.count ? responseCount.data.count : 0
      // jobs
      const responseJobs = await app.$axios.$get(`/api/v1/practice/jobs?locum_detail_id=${user.locum_detail.id}&locum_status=Declined&offset=0&limit=5`)
  ...
  methods: {
    getJobs(page, params) {
      ...
      this.$axios
        .$get(`/api/v1/practice/jobs?locum_detail_id=${this.user.locum_detail.id}&locum_status=Declined&offset=${this.offset}&limit=${this.perPage}`, { params: jobParams })
        .then(res => {
    ...
    show(id) {
      this.$router.push(`/my-banks/all/${this.$route.params.userId}/related-jobs/declined/${id}`)
    },
  }
}
</script>
