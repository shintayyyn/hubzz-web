<template>
  <section class="__jobs-section">
    ...
    <div
      ...
      v-if="getPracticeCompletedJobs.length > 0 && totalPages > 1"
    >
      <AppPagination
        :total="total"
    ...
    <div
      class="shield"
      v-if="$route.name === 'my-banks-favourites-userId-related-jobs-completed-jobId'"
      @click="$router.go(-1)"
    ></div>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    show(id) {
      this.$router.push(
        `/my-banks/favourites/${this.$route.params.userId}/related-jobs/completed/${id}`
      );
    }
  }
};
