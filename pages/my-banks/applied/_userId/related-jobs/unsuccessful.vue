<template>
  <section class="__jobs-section">
    ...
    <div
      class="shield"
      v-if="$route.name === 'my-banks-applied-userId-related-jobs-unsuccessful-jobId'"
    ></div>
    <nuxt-child />
  </section>
</template>
...
export default {
  ...
  async asyncData({ app, params, error }) {
    try {
      ...
      const responseCount = await app.$axios.$get(
        `/api/v1/practice/jobs/count?locum_detail_id=${user.locum_detail.id}&locum_status=Unsuccessful`
      );
      const total =
      ...
      const responseJobs = await app.$axios.$get(
        `/api/v1/practice/jobs?locum_detail_id=${user.locum_detail.id}&locum_status=Unsuccessful&offset=0&limit=5`
      );
      const jobs =
  ...
  methods: {
    getJobs(page, params) {
      ...
      this.$axios
        .$get(
          `/api/v1/practice/jobs?locum_detail_id=${this.user.locum_detail.id}&locum_status=Unsuccessful&offset=${this.offset}&limit=${this.perPage}`,
          { params: jobParams }
        )
        .then(res => {
    ...
    show(id) {
      this.$router.push(
        `/my-banks/applied/${this.$route.params.userId}/related-jobs/unsuccessful/${id}`
      );
    }
  }
};
