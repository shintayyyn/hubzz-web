<template>
  <section v-if="!loading">
    ...
    <div
      class="shield"
      v-if="$route.name.includes('my-banks-applied-userId')"
      @click="$router.push(`/my-banks/applied`)"
    ></div>
    <nuxt-child />
  </section>
</template>
...
export default {
  ...
  watch: {
    $route(value) {
      if (value.name.includes("my-banks-applied-userId")) {
  ...
  methods: {
    ...
    getLocumsCount() {
      this.$axios
        .$get(`/api/v1/practice/locums/count?practice_locum_type=Applied`)
        .then(res => {
