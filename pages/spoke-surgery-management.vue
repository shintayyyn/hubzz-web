<template>
  <div>
    <div class="flex overflow-x-auto border-b-2 border-yellow-500">
      <template v-if="!isStandAloneWithoutHubOrSpoke">
        <nuxt-link
          :to="`/spoke-surgery-management`"
          class="md:mr-5 p-3 text-sm font-bold cursor-pointer whitespace-no-wrap"
          :class="$route.name === 'spoke-surgery-management-index' || ['spoke-surgery-management-index-hub-invitationId', 'spoke-surgery-management-index-create'].includes($route.name) ? 'border-b-2 border-yellow-500' : 'text-gray-600'"
        >My Hub</nuxt-link>
        <nuxt-link
          :to="{ name: 'spoke-surgery-management-spoke-siblings'}"
          class="md:mr-5 p-3 text-sm font-bold cursor-pointer whitespace-no-wrap"
          :class="$route.name.includes('spoke-surgery-management-spoke-siblings') ? 'border-b-2 border-yellow-500' : 'text-gray-600'"
        >My Spoke Siblings</nuxt-link>
        <nuxt-link
          :to="{ name: 'spoke-surgery-management-hub-banks'}"
          class="md:mr-5 p-3 text-sm font-bold cursor-pointer whitespace-no-wrap"
          :class="$route.name.includes('spoke-surgery-management-hub-banks') ? 'border-b-2 border-yellow-500' : 'text-gray-600'"
        >My Hub Banks</nuxt-link>
      </template>
      <nuxt-link
        :to="{ name: 'spoke-surgery-management-invitations-hub'}"
        class="md:mr-5 p-3 text-sm font-bold cursor-pointer whitespace-no-wrap"
        :class="$route.name.includes('spoke-surgery-management-invitations') ? 'border-b-2 border-yellow-500' : 'text-gray-600'"
