      ...
      <div class="shadow-lg rounded-lg bg-gray-300 mt-4 max-w-5xl">
        <div class="flex flex-row flex-wrap justify-start p-4 md:p-8">
          <div class="flex flex-col w-full lg:w-1/4">
            <p class="mr-20 font-semibold">Filename</p>
            <p class="mt-2 text-base">{{variation_terms.filename}}</p>

            <p class="mt-5 mr-20 font-semibold">File last uploaded</p>
            <p
              class="mt-2 text-base"
            >{{$moment(variation_terms.created_at).format('MM/DD/YYYY HH:mm:ss')}}</p>
          </div>
          <div class="w-full mt-5 lg:mt-0 lg:w-3/4">
            <embed
              class="object-contain object-top w-full"
              :class="variation_terms.type == 'image' ? 'image' : 'document h-full '"
              :src="variation_terms.subtype === 'tiff' || variation_terms.subtype === 'msword' ? convertDoc(variation_terms.url) : variation_terms.url"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    convertDoc(document) {
      if (this.variation_terms.subtype === "tiff") {
        return document;
      } else if (this.variation_terms.subtype === "msword") {
