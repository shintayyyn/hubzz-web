...
export default {
      ...
      return [
...

        {
          ...
          column: item =>
            item.ni
              ? "£ " +
                item.ni_amount.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
              : "N/A",
          justify: "start",
          flexGrow: 1,
          flexShrink: 0
  ...
  methods: {
    ...
    filterSearch() {
      ...
      const query = {
        ...
        practice_name_includes: this.practiceNameIncludes
          ? this.practiceNameIncludes
          : undefined,
        tax_number_includes: this.taxNumberIncludes
          ? this.taxNumberIncludes
          : undefined,
    ...
    getLocumInvoiceReportDeductions() {
      ...
      Promise.all([
        this.$axios
          .get("/api/v1/admin/reports/deductions/count", {
            cache: true,
            params: {
              ...params
            }
        ...
        this.$axios
          .get("/api/v1/admin/reports/deductions", {
            cache: true,
            params: {
              ...params,
              order_by: this.orderBy,
    ...
    downloadCsv() {
      ...
      this.$axios
        .post(
          "/api/v1/admin/reports/deductions/generate-key",
          {
        ...
        .then(responses => {
          ...
          window.open(
            `${process.env.API_URL}/api/v1/admin/reports/deductions/csv?token=${token}`
          );
        })
        .catch(err => {
          console.log("err", err);
