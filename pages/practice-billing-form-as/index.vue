  ...
  <section class="relative">
    ...
    <transition name="fade" mode="out-in">
      <div v-if="!initialLoading">
        ...
        <div v-if="locumFormAIdToSign" class="p-2">
          <div class="relative rounded-lg shadow-md px-4 py-8 md:px-8 payment-modal border w-5/6 md:w-1/3">
            ...
            <div class="flex flex-row flex-no-wrap justify-center mt-5">
              <AppButton v-if="!practiceESigningLocumFormA" class="mx-1" :label="'Sign'" :in-style="'padding:5px 10px'" @click="practiceESignLocumFormA" />
    
              <AppButton v-if="practiceESigningLocumFormA" class="mx-1" :label="'Signing...'" :in-style="'padding:5px 10px'" />
        ...
        <transition name="fade" mode="out-in">
          <nuxt-link
            v-if="locumFormAIdToBePaid"
            :to="{ name: 'practice-billing-invoices-from-locums' }"
            class="shield"
            @click.native="locumFormAIdToBePaid = null"
          />
        </transition>
...
export default {
  ...
  mounted () {
    this.$socket.on('Practice Notification Locum Form A Paid', this.getLocumInvoiceRealTime)
    this.$socket.on('Practice Notification Locum Form A Sent To Practice', this.getLocumInvoiceRealTime)

    this.initialLoading = true
    this.getLocumFormAsPromiseAll().catch((err) => {
  ...
  destroyed () {
    this.$socket.removeListener('Practice Notification Locum Form A Paid', this.getLocumInvoiceRealTime)
    this.$socket.removeListener('Practice Notification Locum Form A Sent To Practice', this.getLocumInvoiceRealTime)
  },
  ...
  methods: {
    ...
    getLocumFormAsPromiseAll () {
      ...
      return Promise.all([
        ...
        .then(([responseTotal, responseJobParts,]) => {
          ...
          this.locumFormAs = responseJobParts.data.locum_form_as.map(jobPart => {
            return {
              ...jobPart,
    ...
    getLocumFormAs () {
      return this.$axios
        ...
        .then(res => {
          let locumFormAs = res.data.locum_form_as

          this.locumFormAs = locumFormAs.map(jobPart => {
            return {
              ...jobPart,
              under_parent_practice: jobPart.parent_practice_id ? "Yes" : "No",
            }
    ...
    formAPaidByPractice () {
      ...
      if (this.formErrors.length === 0) {
        this.$axios.put(`/api/v1/practice/locum-form-as/${this.locumFormAIdToBePaid}/paid`, {
          ...
          const jobPart = this.locumFormAs.find(jobPart => jobPart.id === locumFormA.id)

          const index = this.locumFormAs.findIndex(jobPart => jobPart.id === locumFormA.id)

          if (jobPart && index > -1) {
            jobPart.paid_by_practice = locumFormA.paid_by_practice
            jobPart.paid_by_practice_formatted = locumFormA.paid_by_practice_formatted
            jobPart.paid_by_practice_at = locumFormA.paid_by_practice_at
            jobPart.paid_by_practice_at_formatted = locumFormA.paid_by_practice_at_formatted
    ...
    practiceESignLocumFormA () {
      ...
      this.$axios
        .put(`/api/v1/practice/locum-form-as/${this.locumFormAIdToSign}/e-sign`, formData)
        .then((response) => {
          const locumFormA = response.data.data.locum_form_a
