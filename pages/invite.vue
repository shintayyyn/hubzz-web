    ...
    >
      <div class="flex overflow-x-auto items-center">
        ...
        <nuxt-link
          ...
          class="mr-5 px-3 py-2 text-sm font-bold cursor-pointer"
          :class="
            $route.name === 'invite-invite-practices'
              ? 'border-b-4 border-yellow-500'
              : 'text-gray-600'
          "
        >
...
export default {
  ...
  mounted() {
    this.$axios
      .get("/api/v1/user-invites/referral-code", { cache: true })
      .then(response => {
        if (
          response.data &&
          response.data.data &&
          response.data.data.user_referral_code
        ) {
          this.referralCode =
            response.data.data.user_referral_code.referral_code;
        }
      });
  }
};
