...
export default {
  ...
  watch: {
    modal(open) {
      // Fetch dropdown data only when the modal is opened
      if (!open) return;
  ...
  mounted() {
    ...
    // NOTE: dropdown data is fetched lazily when the modal/type is selected
  },
  ...
  methods: {
    ...
    getExpenseTotal(data) {
      ...
      return this.$axios
        .$get(`/api/v1/locum/locum-expenses`, {
          params: {
    ...
    async editExpenseReports(id) {
      ...
      if (foundExpenseReport) {
        ...
        this.form.total = foundExpenseReport.total
          ? foundExpenseReport.total.toString()
          : "0";
    ...
    exportExpenseReportAsPdf() {
      ...
      const formattedDateStart = this.$moment(dateStart, "YYYY-MM-DD").format(
        "DD/MM/YYYY"
      );
      const formattedDateEnd = this.$moment(dateEnd, "YYYY_MM_DD").format(
        "DD_MM_YYYY"
      );
      ...
      this.exporting = true;
      this.$axios
        .get("/api/v1/locum/locum-expenses/pdf", {
          params: {
            date_start: dateStart,
            date_end: dateEnd
          },
