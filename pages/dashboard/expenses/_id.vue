      ...
      <div class="flex flex-col mb-4 relative">
        ...
        <div class="max-w-3xl border p-4 rounded-lg">
          ...
          <AppInput
            ...
            :placeholder="
              gettingPrivatePractices
                ? 'Loading private practices...'
                : 'Select...'
            "
            :error="
            ...
            :required="true"
            :disabled="gettingPrivatePractices"
          />
...
export default {
  ...
  watch: {
    "form.type"() {
      this.formError = this.formError.filter(
        formError => formError.field !== "type"
      );
    ...
    "form.practice_id"() {
      this.formError = this.formError.filter(
        formError => formError.field !== "practice_id"
      );
    ...
    "form.private_practice_id"() {
      this.formError = this.formError.filter(
        formError => formError.field !== "private_practice_id"
      );
    ...
    "form.total"() {
      this.formError = this.formError.filter(
        formError => formError.field !== "total"
      );
  ...
  methods: {
    async fetchExpense() {
      try {
        ...
        const res = await this.$axios.get(
          `/api/v1/locum/locum-expenses/${this.expenseId}`
        );
        this.expense = res.data.data.locum_expense;

        this.form.type = this.expense.type;
        this.form.practice_id = this.expense.practice_id;
        this.form.private_practice_id = this.expense.private_practice_id;
        this.form.description = this.expense.description;
        this.form.total = this.expense.total
          ? this.expense.total.toString()
          : "0";
    ...
    async fetchPrivatePractices() {
      if (this.gettingPrivatePractices || this.privatePractices.length) return;
    ...
    async fetchPractices() {
      if (this.gettingPractices || this.practices.length) return;
