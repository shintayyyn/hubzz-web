...
export default {
  ...
  computed: {
   ...
   mounted () {
    ...
    this.$axios.get(`/api/v1/locum/locum-expenses/${this.expenseId}`).then(res => {
      ...
      this.form.total = this.expense.total ? this.expense.total.toString() : '0'
  ...
  methods: {
      ...
      if (!this.formError.length) {
        try {
          ...
          let response = await this.$axios.$put(`/api/v1/locum/locum-expenses/${this.expenseId}`, formData)
