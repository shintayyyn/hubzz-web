				...
				<div class="shadow-lg rounded-lg bg-gray-300 mt-5 max-w-5xl">
					<div class="flex flex-row flex-wrap justify-start p-4 md:p-8">
						<div class="flex flex-col w-full lg:w-1/4 pr-4">
							...
							<p
								class="mt-2 text-sm md:text-base"
							>{{mandatory_training.file ? $moment(mandatory_training.file.created_at).utc().format('DD/MM/YYYY HH:mm:ss') : null}}</p>
							<div v-if="mandatory_training.status === 'Approved'">
								<p class="mt-5 font-bold text-lg">Expiration Date</p>
								<p
									class="mt-2 text-sm md:text-base"
								>{{mandatory_training.expired_at ? $moment(mandatory_training.expired_at).utc().format('DD/MM/YYYY HH:mm:ss') : null}}</p>
							</div>
							<div v-if="mandatory_training.status === 'Rejected'">
								...
								>{{ formError.find(item=> item.field === 'expiry_date').message }}</template>
						...
						<div class="mt-5 lg:mt-0 w-full lg:w-3/4" v-if="mandatory_training.file">
							<embed
