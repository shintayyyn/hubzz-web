      ...
      <template v-if="$auth.user.domain === 'Locum' && !loading">
        ...
        >
          ...
          <template v-slot:status_slot="slotProps">
            <div class="flex items-center justify-center">
              {{
                slotProps.item.status === "Closed" &&
                  slotProps.item.job_posting_status === "Available"
                  ? "Rejected"
                  : slotProps.item.status
              }}
            </div>
        ...
        permanent_jobs_for_practice = permanent_jobs_for_practice.map(
          permanent_job => {
            if (permanent_job.permanent_job_applications.length > 0) {
              console.log(
                permanent_job.permanent_job_applications.filter(
                  permJobApp => permJobApp.application_status === "Rejected"
                )
              );
