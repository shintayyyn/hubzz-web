			...
			<template v-if="$auth.user.domain === 'Practice' && !loading">
				...
				>
					<template v-slot:salary_slot="slotProps">
						...
						>{{ slotProps.item.salary_amount | currency }}</template>
						<template v-else>N/A</template>
					</template>

					<template
						v-slot:date_posted="slotProps"
					>{{ $moment(slotProps.item.date_posted).format("DD/MM/YYYY") }}</template>

					<template
						v-slot:date_closing="slotProps"
					>{{ $moment(slotProps.item.date_closing).format("DD/MM/YYYY") }}</template>
				...
				>No {{ $route.query.status ? $route.query.status : 'Available' }} Jobs Found.</p>
			</template>
