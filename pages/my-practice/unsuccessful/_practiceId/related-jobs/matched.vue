<template>
  <section class="relative __jobs-section">
    ...
    <div v-if="getLocumMatchedJobs.length > 0" class="overflow-x-auto overflow-y-hidden">
    ...
    <div
      class="shield"
      v-if="$route.name === 'my-practice-unsuccessful-practiceId-related-jobs-matched-jobId'"
    ></div>
    <nuxt-child />
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    show(id) {
      this.$router.push(
        `/my-practice/unsuccessful/${this.$route.params.practiceId}/related-jobs/matched/${id}`
      );
    }
  }
};
