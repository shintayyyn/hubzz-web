<template>
  <section class="relative __jobs-section">
    ...
    <div
      ...
      v-if="!loadingJobs && getLocumDeclinedJobs.length === 0"
    >You have not yet declined a job</div>
    <div v-if="getLocumDeclinedJobs.length > 0" class="overflow-x-auto overflow-y-hidden">
      <JobTable :columns="columns" :jobs="getLocumDeclinedJobs" @sortBy="sortBy" @show="show" />
    </div>
    <div class="absolute bottom-0 w-full" v-if="getLocumDeclinedJobs.length > 0 && totalPages > 1">
      <AppPagination
        :total="total"
    ...
    <div
      class="shield"
      v-if="$route.name === 'my-practice-unsuccessful-practiceId-related-jobs-declined-jobId'"
      @click="$router.go(-1)"
    ></div>
    <nuxt-child />
  </section>
...
export default {
  ...
  created() {
    ...
    setTimeout(() => {
      this.$store.commit("jobs/CLEAR_LOCUM_DECLINED_BADGE");
    }, 1000);
  },
  methods: {
    getJobsCount() {
    ...
    show(id) {
      this.$router.push(
        `/my-practice/unsuccessful/${this.$route.params.practiceId}/related-jobs/declined/${id}`
      );
    }
  }
};
