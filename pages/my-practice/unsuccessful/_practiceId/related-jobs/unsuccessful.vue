<template>
  <section class="relative __jobs-section">
    ...
    <div
      class="shield"
      v-if="$route.name === 'my-practice-unsuccessful-practiceId-related-jobs-unsuccessful-jobId'"
      @click="$router.go(-1)"
    ></div>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    show(id) {
      this.$router.push(
        `/my-practice/unsuccessful/${this.$route.params.practiceId}/related-jobs/unsuccessful/${id}`
      );
    }
  }
};
