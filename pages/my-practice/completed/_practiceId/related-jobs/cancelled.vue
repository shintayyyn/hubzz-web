<template>
  <section class="relative __jobs-section">
    ...
    <div
      class="shield"
      v-if="$route.name === 'my-practice-completed-practiceId-related-jobs-cancelled-jobId'"
      @click="$router.go(-1)"
    ></div>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    show(id) {
      this.$router.push(
        `/my-practice/completed/${this.$route.params.practiceId}/related-jobs/cancelled/${id}`
      );
    }
  }
};
