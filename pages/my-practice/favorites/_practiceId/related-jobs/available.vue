<template>
  <section class="relative __jobs-section">
    ...
    <div v-if="getLocumAvailableJobs.length > 0" class="overflow-x-auto overflow-y-hidden p-2">
    ...
    <div
      class="shield"
      v-if="$route.name === 'my-practice-favorites-practiceId-related-jobs-available-jobId'"
      @click="$router.go(-1)"
    ></div>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    show(id) {
      this.$router.push(
        `/my-practice/favorites/${this.$route.params.practiceId}/related-jobs/available/${id}`
      );
    }
  }
};
