<template>
  <section class="relative __jobs-section">
    ...
    ></AppTable>
    <div
      v-if="!getLocumDeclinedJobs.length && !loadingJobs"
      class="flex justify-center py-4"
    >You do not have any declined a job</div>
    <transition name="fade" mode="out-in">
      <div
        class="shield"
        v-if="$route.name === 'my-practice-applied-practiceId-related-jobs-declined-jobId'"
        @click="$router.push(`/my-practice/applied/${$route.params.practiceId}/related-jobs/declined`)"
      ></div>
    </transition>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    filterOut: debounce(function({ field, value }) {
      ...
      this.$store.commit("jobs/SET_LOCUM_DECLINED_JOBS", []);
    ...
    show(item) {
      this.$router.push(
        `/my-practice/applied/${this.$route.params.practiceId}/related-jobs/declined/${item.id}`
      );
    },
    onSelect(value) {
