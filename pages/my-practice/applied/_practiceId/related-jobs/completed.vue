<template>
  <section class="relative __jobs-section">
    ...
    >You have not yet completed any jobs</div>
    <transition name="fade" mode="out-in">
      <div
        class="shield"
        v-if="$route.name === 'my-practice-applied-practiceId-related-jobs-completed-jobId'"
        @click="$router.push(`/my-practice/applied/${$route.params.practiceId}/related-jobs/completed`)"
      ></div>
    </transition>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    show(item) {
      this.$router.push(
        `/my-practice/applied/${this.$route.params.practiceId}/related-jobs/completed/${item.id}`
      );
    },
    onSelect(value) {
