<template>
  <section class="relative __jobs-section">
    ...
    <AppTable
      v-if="getLocumCancelledJobs.length > 0"
      :total="total"
      :items="getLocumCancelledJobs"
    ...
    ></AppTable>
    <div
      v-if="!getLocumCancelledJobs.length && !loadingJobs"
      class="flex justify-center py-4"
    >You do not have any cancelled job</div>
    <transition name="fade" mode="out-in">
      <div
        class="shield"
        v-if="$route.name === 'my-practice-applied-practiceId-related-jobs-cancelled-jobId'"
        @click="$router.push(`/my-practice/applied/${$route.params.practiceId}/related-jobs/cancelled`)"
      ></div>
    </transition>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    filterOut: debounce(function({ field, value }) {
      ...
      this.$store.commit("jobs/SET_LOCUM_CANCELLED_JOBS", []);
    ...
    show(item) {
      this.$router.push(
        `/my-practice/applied/${this.$route.params.practiceId}/related-jobs/cancelled/${item.id}`
      );
    },
    onSelect(value) {
