<template>
  <section class="relative __jobs-section">
    ...
    ></AppTable>
    <div
      v-if="!getLocumMatchedJobs.length && !loadingJobs"
      class="flex justify-center py-4"
    >There are no matched jobs nearby and suited for you at this time</div>
    <transition name="fade" mode="out-in">
      <div
        class="shield"
        v-if="$route.name === 'my-practice-applied-practiceId-related-jobs-matched-jobId'"
        @click="$router.push(`/my-practice/applied/${$route.params.practiceId}/related-jobs/matched`)"
      ></div>
    </transition>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    filterOut: debounce(function({ field, value }) {
      ...
      this.$store.commit("jobs/SET_LOCUM_MATCHED_JOBS", []);
    ...
    show(item) {
      this.$router.push(
        `/my-practice/applied/${this.$route.params.practiceId}/related-jobs/matched/${item.id}`
      );
    },
    onSelect(value) {
