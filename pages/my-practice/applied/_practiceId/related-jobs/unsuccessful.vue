<template>
  <section class="relative __jobs-section">
    ...
    <transition name="fade" mode="out-in">
      <div
        class="shield"
        v-if="$route.name === 'my-practice-applied-practiceId-related-jobs-unsuccessful-jobId'"
        @click="$router.push(`/my-practice/applied/${$route.params.practiceId}/related-jobs/unsuccessful`)"
      ></div>
    </transition>
    <nuxt-child />
  </section>
...
export default {
  ...
  methods: {
    ...
    filterOut: debounce(function({ field, value }) {
      ...
      this.$store.commit("jobs/SET_LOCUM_UNSUCCESSFUL_JOBS", []);
    ...
    show(item) {
      this.$router.push(
        `/my-practice/applied/${this.$route.params.practiceId}/related-jobs/unsuccessful/${item.id}`
      );
    },
    onSelect(value) {
