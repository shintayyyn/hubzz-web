  ...
  methods: {
    getPracticeInvoiceFinanceReports() {
      ...
      Promise.all([
        ...
        this.$axios
          .get(
            `/api/v1/practice/practice-invoice-finance-reports/${year}/${month}`,
            { cache: true }
          )
          .then(responses => {
    ...
    exportPracticeInvoiceFinanceReportsAsPdf() {
      ...
      this.$axios
        .post(
          `/api/v1/practice/practice-invoice-finance-reports/${year}/${month}/generate-key`
        )
        .then(responses => {
          const token = responses.data.data.token;

          window.open(
            `${process.env.API_URL}/api/v1/practice-invoice-finance-reports/pdf/${filename}?token=${token}`
          );
        })
        .catch(err => {
          console.log("err", err);
