  ...
  <div class="max-w-5xl h-screen">
    ...
    <section class="pt-2">
      <div class="flex flex-row flex-wrap justify-start items-center my-2">
        ...
        <div
          class="text-xs sm:text-sm mx-1 py-1 px-3 border-2 rounded-lg font-bold flex items-center focus:outline-none bg-sunglow border-sunglow"
        >
          Platform
        </div>
      </div>
      ...
      >
        {{
          "* This invoice has been marked as paid on " +
            practiceInvoice.paid_at_in_gb_formatted
        }}
      </div>
      <div
      ...
      >
        <div>
          {{
            `* This invoice has been marked Invalid on ${practiceInvoice.unpaid_at_in_gb_formatted}`
          }}
        </div>
    ...
    try {
      ...
      for (let i = 0; i < practiceInvoiceItems.length; i++) {
        const newItem = {
          ...
          total_hours: practiceInvoiceItems[i].job_part
            ? practiceInvoiceItems[i].job_part.final_hours / 60
            : null
        };
