	...
	<section class="availability-section">
		...
		<div class="availability-shift rounded-lg shadow-lg mt-5">
			<div class="relative w-full p-5">
				...
				<div class="mx-2 mt-4">
					<AppButton :label="'Update'" @click="update" :disabled="loading" style="padding: 8px 16px;" />
				</div>
			</div>
		</div>
		...
		<template v-if="['Active', 'Dormant'].includes($auth.user.status)">
			...
			<transition name="fade" mode="out-in">
				<nuxt-link :to="'/availability'">
					<transition name="fade" mode="out-in">
						<div class="shield" v-if="['availability-id','availability-create'].includes($route.name)"></div>
					</transition>
				</nuxt-link>
			</transition>
			<nuxt-child
