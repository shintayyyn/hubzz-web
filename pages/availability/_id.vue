      ...
      <div class="mt-4">
        <div class="relative bg-white rounded-lg shadow-lg p-4 md:p-8">
          ...
          <div class="flex flex-col w-full my-6">
            ...
            >
              {{ $moment($store.state.availability.selected_date, 'YYYY-MM-DD').format('DD/MM/YYYY') }}
            </div>
          </div>
          <div class="flex flex-row flex-wrap items-center justify-between mt-4 relative">
            ...
            >
              {{ formError.find(item => item.field === 'shift_id').message.charAt(0).toUpperCase() + formError.find(item => item.field === 'shift_id').message.slice(1).replace(/_/g, " ") }}
            </div>
          </div>
          <div
