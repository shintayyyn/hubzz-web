export default {
    async initializeJobListener({ commit }) {
        ...
        this.$socket.on('Locum Notification Job Part Approved', async (job_part) => {
            const response = await this.$axios.$get(`/api/v1/locum/job-parts/${job_part.id}`)
            if (response.data && response.data.job_part) {
                commit('ADD_LOCUM_JOB_NOTIFICATION', { ...response.data.job_part, notificationType: 'Locum Notification Job Part Approved' })
            }
        })
        this.$socket.on('Locum Notification Job Updated', async (job) => {
            const response = await this.$axios.$get(`/api/v1/locum/jobs/${job.id}`)
            if (response.data && response.data.job) {
                commit('ADD_LOCUM_JOB_NOTIFICATION', { ...response.data.job, notificationType: 'Locum Notification Job Updated' })
            }
        })
        ...
        // withdrawn
        this.$socket.on('Practice Notification Job Part Approved', async (job_part) => {
            const response = await this.$axios.$get(`/api/v1/practice/job-parts/${job_part.id}`)
            if (response.data && response.data.job_part) {
                commit('ADD_PRACTICE_JOB_NOTIFICATION', { ...response.data.job_part, notificationType: 'Practice Notification Job Part Approved' })
            }
        })
        ...
        // approved
        this.$socket.on('Practice Notification Job Updated', async (job) => {
            const response = await this.$axios.$get(`/api/v1/practice/jobs/${job.id}`)
            if (response.data && response.data.job) {
                commit('ADD_PRACTICE_JOB_NOTIFICATION', { ...response.data.job, notificationType: 'Practice Notification Job Updated' })
            }
        })
    },
