export default {
  ...
  // NOTIF
  ADD_LOCUM_JOB_NOTIFICATION (state, job) {
    let index = state.locum_job_notifications
      .findIndex(locumJobNotification => locumJobNotification.id === job.id)

    if (index > -1) {
      state.locum_job_notifications.splice(index, 1, job)
    } else {
      state.locum_job_notifications.unshift(job)
    }
  },

  REMOVE_LOCUM_JOB_NOTIFICATION (state, jobid) {
    let index = state.locum_job_notifications
      .findIndex(locumJobNotification => locumJobNotification.id === jobid)

    if (index > -1) {
      state.locum_job_notifications.splice(index, 1)
    }
  },
  ...
  // },
  ADD_OR_UPDATE_LOCUM_UNAVAILABILITIES (state, updatedUnavailabilities) {
    updatedUnavailabilities.forEach(updatedUnavailability => {
      const unavailability = state.locum_unavailabilities.find(({ date, }) => date === updatedUnavailability.date)
      
      if (unavailability) {
        unavailability.shifts = updatedUnavailability.shifts
      } else {
        state.locum_unavailabilities.push(updatedUnavailability)
      }
    })
  },
